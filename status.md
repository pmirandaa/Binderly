...
- **Q-014** (raised by T-SC-ANN-INDEX, PR #76; **Pablo answered 2026-05-20**: "choose accuracy, but also keep latency as low as possible"): pure-JS dot-product brute-force ANN projects to ~140 ms per query on a Pixel 6-class device at full production catalog (~30 k × 576), exceeding the 30 ms scanner stage budget. Pablo's directive maps to **Option 2 — native SIMD inner loop** (`vDSP_distancesq` on iOS Accelerate.framework + ARM Neon on Android via JNI). Preserves 100 % recall (same brute-force, vectorised), ~10-20× faster than pure JS. Substantively its own task — proper Expo Module with Swift + Kotlin + autolinking + Expo plugin + TS feature-detection fallback. **Logged as #FU-29 / `T-SC-ANN-NATIVE`** in Known Follow-ups; the file at `apps/mobile/src/scanner/ann/search.ts` is the transparent swap point. Non-blocking for v1 beta (~3-5 k printings stays inside the 30 ms budget on pure-JS); critical before the catalog grows past ~10 k printings. **Status: deferred to #FU-29; T-SC-MATCH ships against the existing pure-JS surface.**
...
29. **`T-SC-ANN-NATIVE` (Q-014 resolution, proposed).** Pablo answered Q-014 with "choose accuracy, but also keep latency as low as possible" — Option 2 (native SIMD inner loop) is the unambiguous match. Ship as a proper Expo Module: Swift package wrapping `Accelerate.framework`'s `vDSP_distancesq` (iOS) + Kotlin AAR with JNI bindings to ARM Neon intrinsics (Android, `vmlaq_f32` family) + Expo plugin registering both in the autolinking chain + TS wrapper with feature-detection fallback to the existing pure-JS path at `apps/mobile/src/scanner/ann/search.ts`. Estimated ~50 LOC native per platform + ~100 LOC TS wrapper + ~50 tests. Critical before the catalog grows past ~10 k printings; non-blocking for v1 beta launch (~3-5 k printings comfortably inside the 30 ms budget on pure-JS). **Logged as #FU-29.**
30. **Cosmetic chore for Pablo: orphan worktree dirs at `/Users/pmiranda/Stuff/binderly-wt-T-*`.** Git no longer tracks them as worktrees; safe to `rm -rf`. pnpm-store residue blocks sandbox `rm`. Affected: T-DL-SEED-INGEST, T-FN-LINT-CONFIG, T-M-COLLECTION, T-W-BROWSE, T-W-COLLECTION, T-W-CUSTOM, T-W-SHAREABLE-PUBLIC, T-W-SMART, T-BE-EDGE-FUNCTIONS-V2, T-W-API-V2-WIRING, T-M-API-V2-WIRING, T-SC-CAMERA, T-SC-EMBED-MODEL, T-BE-Q013-CLEANUP, T-DL-RLS-PG-STAT-REVOKE, T-SC-ANN-INDEX, T-SC-DETECT.
