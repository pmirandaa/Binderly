# Build status — **Iter 31 closed.** **Scanner stage CLOSED (6/6).** Stage 07 grading **9/10** (T-GR-AGGREGATE merged). **Stage 09 offline-sync CLOSED (3/3)** (T-OF-CONFLICTS merged). Q-016 resolved (T-OF-QUEUE; set_logo_url denormalised); Q-017 raised + closed in iter 30 (resolved by #FU-44). Q-018 raised + closed in iter 31 (T-GR-AGGREGATE; confidence-band categorical thresholds + centering grade-hint mapping documented). Q-019 raised + closed in iter 31 (T-OF-CONFLICTS; api-client single-GET gap + repository upsertFromServer non-API both worked around via in-module adapters; #FU-49 logs the api-client cleanup).
...
**Stage 08 shareables polish:** 1 / 3 merged (T-SH-OG-IMAGES — iter 32). Dynamic OG image route at `/api/og/share/[handle]/[slug]` ships the rich card-grid hero (1200×630 PNG; gradient background; top-left wordmark; centre title; 4-tile lower row from member thumbnails with 2 s per-image timeout + brand-coloured silhouette fallback; footer URL + stat line). Cache `public, max-age=3600, s-maxage=86400, stale-while-revalidate=604800` on success, `public, max-age=3600` on fallback. Unknown handle / `is_public=false` / network failure all serve a generic Binderly-branded fallback PNG (NOT 404 — social platforms cache 404s aggressively). Public-page `generateMetadata` now wires `<meta property="og:image">` + Twitter card images at the new route. T-SH-CONFIG-MODEL + T-SH-THEMES remaining.
...
- T-OF-QUEUE — `43a8afa` (**Stage 09 offline-sync 2/3**; ships `apps/mobile/src/sync/queue/` — `sync_queue` SQLite table (v2 migration), enqueue subscribers for all 3 repositories via `onLocalWrite`, translator mapping 11 operation variants to `@binderly/api-client`, `ReplayEngine` state machine (idle/replaying/paused), exponential backoff `30s×2^n` capped 1 h, dead-letter seam for T-OF-CONFLICTS after 10 attempts, connectivity wiring via `@react-native-community/netinfo`, `startSync(collection)` wiring shim; **+99 mobile tests (1198 total)**; resolved Q-016 (Option 1: `set_logo_url` denormalised into `printing_lite`); #FU-41 scope refined to background browse-history pre-warming) — **iter 30 cap**
