...
- T-DL-TOOLING-HYGIENE — `agent/T-DL-TOOLING-HYGIENE` (**deferred data-layer + tooling hygiene backlog burndown**; clears 7 long-deferred follow-ups, all verified locally — no new SQL migration, `_journal.json` untouched except graceful-skip logic. **#FU-1**: `@binderly/db` `db:generate`/`db:migrate` rewritten as plain-ESM `scripts/*.mjs` (drops the `tsx` runtime that hit sandbox IPC-pipe failures); pure helpers extracted + first real vitest suite for the package (`__test__/`, **24 tests**); `migrate.mjs` verified live against Supabase :54322 (applied 25 migrations). **#FU-7**: `migrate.mjs` degrades gracefully on a missing/empty journal (clear notice + exit 0) while still erroring on journal-references-missing-SQL corruption. **#FU-2**: one shared `RawPriceObservation` (+ `rawPriceObservationSchema`) hoisted to `data-pipeline/src/types.ts`; aggregator re-exports it (same symbol, no barrel collision), EBAY-BROWSE imports it directly, `RawEbayBrowsePriceObservation` retired; `parseConfidence` standardised as a `numeric(3,2)` string to match the DB column. **#FU-3**: new `data-pipeline/src/load-env.ts` auto-loads `.env` (package-local then monorepo root, never clobbering set vars), imported first in all 5 `scripts/*.ts` + covered by `load-env.test.ts` (7 tests); README repointed from the Compose :5433 stub to Supabase :54322 where migrations live. **#FU-4**: `^build` already on build/test/typecheck (already-done), but `pnpm seed` bypassed turbo — added a `seed` task (`dependsOn: ["^build"]`) + root `seed` script. **#FU-9**: `.nvmrc` + root `engines.node` → `22.13.0` (installable). **#FU-10**: task-id regex relaxed to `^T-[A-Z]+-[A-Z0-9-]+$` across the 3 worktree scripts + `scripts/README.md` + `ci-pr-title.yml` (single-letter scopes like `T-M-SHELL` now pass). Local battery green: install / build 12·12 / lint 13·13 / typecheck 21·21 / `@binderly/db` 24·24 / data-pipeline **1108·1108** / verify-rls **131 pass, 0 fail** (prior known-fail baseline was 2 on `v_pg_stat_statements_top_queries` — #FU-28, untouched; now 0, not worse)) — **iter 34**
