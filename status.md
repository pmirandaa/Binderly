...
- T-GR-DATA-HYGIENE — `26d334a` (#107) (**grading data-hygiene cluster — #FU-44 + #FU-40 + #FU-56**; Python-only, no DB migration. **#FU-44** `ml_common` subgrade-key generalisation (closes Q-017): renamed `LabelledGradingSample.corners_score`→`subgrade_score` + `is_labelled_for_corners()`→`is_labelled()`; parametrised `PSADataLoader`/`EbayDataLoader`/`AuctionDataLoader`/`MergedDataLoader` on `subgrade_key: Literal['corners','edges','surface']` (default `corners`); deleted the duplicate `EdgesMergedDataLoader`/`SurfaceMergedDataLoader` + `EdgesLabelledSample` so all 3 sub-grade trees share one loader path — behaviour identical. **#FU-40** `T-GR-DATA-PRINTING-MATCH`: new pure-Python `apps/api-python/grading/matching/` (no new deps) — `text.py` observation-title parser (grade-prefix strip + card-number/variant extraction + name tokenise), `matcher.py` `PrintingMatcher` (difflib composite name/set/number/variant score → `EXACT`/`FUZZY`/`AMBIGUOUS`/`NO_MATCH`; ambiguous + no-match leave `printing_id` NULL on purpose), `backfill.py` **dry-run-by-default** job (`binderly-backfill-printing-match`; live UPDATE via Supabase PostgREST behind an env gate) over the 3 observation tables + JSON fixtures + unit tests (exact/fuzzy/ambiguous/no-match). **#FU-56** cross-company grade calibration **scaffold**: new `apps/api-python/grading/calibration/` — PSA-anchored per-company affine placeholder map (numpy dot, low-confidence heuristic constants, clamped `[1,10]`) + `GradeCalibrator`/`normalize_grade`; learned fit deferred to #FU-60. **984 pytest pass / 1 skip** (live-image gate); no TS contract touched (lint/typecheck/build unaffected). Raised **Q-023** (calibration constants are unvalidated placeholders; #FU-60)) — **iter 35**
- T-BE-READS-WRITES — `8a4bb5e` (#106) (**backend reads/writes follow-up cluster — #FU-49 + #FU-55 + #FU-54**; **#FU-49** single-GET collection items closing the Q-019 non-API-read gap: Edge `GET /me/collection/:id` + `GET /me/custom-collections/:id/items/:printingId` (RLS-gated, `NOT_FOUND` on miss; the `:id` route ordered after the literal `completion` route so it can't shadow it) + api-client `collection.getCollectionItem({id})` / `getCustomCollectionItem({customCollectionId, printingId})` — the T-OF-CONFLICTS `ServerFetcher` page-walk swap remains, spun out to #FU-59. **#FU-55** pro-gated Edge `POST /me/community-submissions` handler backing the already-pinned contract + client: auth → fail-closed RC Pro gate (reuses `resolveEntitlements`) → mirrored Zod validate → cert-normalise → insert into `community_submission` idempotent on `(user_id, grade_company, cert_number_normalized)` → `alreadySubmitted`. **#FU-54** entitlement-fallback observability: dependency-light structured `console.warn` default sink + injectable `onFallback` at the `@binderly/entitlements` fallback site (a throwing hook can't break the fail-closed read). **No new DB migration** (uses the existing `community_submission` 0025/0026). Local CI green (lint/typecheck/test/build + edge-functions vitest 356 / tsc); new tests — entitlements +6, api-client +7, edge collection +6 / customCollections +3 / communitySubmissions +10 / contracts +9) — **iter 35**
