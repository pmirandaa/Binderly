# Build status — Iter 27 closed. **Scanner stage CLOSED (6/6).** Stage 07 grading 2/10. Q-014 deferred to #FU-29.
...
Iter 27 closed Stage 06 scanner (6 / 6) with T-SC-UX shipping the screen + match overlay + disambiguation chooser + stability indicator + stack-mode review + undo toast; and progressed Stage 07 grading to 2 / 10 with T-GR-CENTERING shipping a Python authoritative PSA centering measurement + a TypeScript contract + session-routing seam (closes #FU-32). Workers reported clean local CI but main needed two stitching fixes the sibling-pair sequence introduced: (a) workspace packages `@binderly/set-completion` / `@binderly/smart-collection-dsl` / `@binderly/pricing-display` had no `dist/` on first checkout of main — resolved by `pnpm build` (turbo task graph); (b) `GradingCaptureScreen.test.tsx` asserted the old `/grading/capture/review` route, hotfixed at `1f90a1e` to assert the new `/grading/centering?sessionId=<id>` contract T-GR-CENTERING introduced.
