# Build status — **Iter 35.** ✅ **Scanner CLOSED (6/6).** ✅ **Stage 07 grading CLOSED (10/10).** ✅ **Stage 09 offline-sync CLOSED (3/3).** ✅ **Stage 10 paywall/billing CLOSED (4/4)** (RevenueCat + Paddle + `@binderly/entitlements` + `@binderly/feature-flags` gating). ✅ **Stage 08 shareables CLOSED (3/3)** (themed public shareable pages shipped — T-SH-THEMES). Stage 11 deployment **5/6 scaffolded** (Vercel/Fly/Supabase-prod/R2-prod/EAS inert until secrets per `infra/DEPLOYMENT_SECRETS.md`; only T-DP-MONITORING remains). **81/86 tasks merged.** Open: Q-021 (api-python needs a FastAPI entrypoint before Fly go-live; #FU-53); Q-022 (mobile release cadence + OTA strategy; #FU-58); Q-023 (cross-company grade calibration constants are unvalidated placeholders; #FU-60); Q-024 (public share payload lacks owner tier → server-side free-theme downgrade can't enforce yet; #FU-61).
...
- T-DP-EAS — `36da110` (#105) (**Stage 11 deployment 5/6 scaffolded**; EAS build/submit for the Expo app, inert until Expo/Apple/Google credentials land. `apps/mobile/eas.json` ships development/preview/production build profiles + a production submit profile (Expo SDK 52; `cli.appVersionSource: remote` so EAS owns the iOS buildNumber / Android versionCode, production `autoIncrement`); no hardcoded secrets / Apple Team IDs / key paths — store credentials referenced via env placeholders (`$ASC_API_KEY_*`, `$GOOGLE_SERVICE_ACCOUNT_KEY_PATH`). `.github/workflows/deploy-mobile.yml` guards `eas build`/`eas submit` on an `EXPO_TOKEN` secret (skipped, never failed, when absent — same guard pattern as deploy-web/fly/db); manual-`workflow_dispatch` only (native builds cost EAS build minutes) with opt-in store submit. `infra/DEPLOYMENT_SECRETS.md` §5 + `infra/eas/README.md` document the secrets (EXPO_TOKEN, ASC API key, Google Play SA JSON) + first-run runbook. No DB migrations; local CI green (lint + typecheck + build + **1631 mobile vitest tests**). Raised Q-022 (mobile release cadence + OTA; #FU-58). Unblocks T-DP-MONITORING) — **iter 35**
- T-PB-GATING — `1e93afe` (#103) (**Stage 10 paywall / billing ✅ 4 / 4 — CLOSED**; ships free/pro feature gating across web + mobile on top of last iter's `@binderly/entitlements` keystone. New framework-agnostic `packages/feature-flags/` (`@binderly/feature-flags`): pure `evaluateGate(entitlement, feature)` + `evaluateLimitGate(entitlement, resource, currentCount)` decision core (built on `canUseFeature`/`withinFreeLimit`; `GateResult = {allowed:true} | {allowed:false, reason:'requires_pro'|'free_limit_reached', feature, limit?}`; `resourceToFeature` maps a count-limited resource → its `unlimited_*` feature; fail-closed is the *caller's* job — the core is total). Web glue (`apps/web/lib/gating/`): `useEntitlement` (TanStack Query over `getMyEntitlements()`, loading/error → free), `useGate`/`useLimitGate`, `<Gate>`/`<GateSkeleton>`/`<UpgradePrompt>` (CTA → `/billing`). Mobile glue (`apps/mobile/src/lib/gating/`): same hook surface reading RC directly via the billing module's `useEntitlementsQuery` (no redundant HTTP hop, correct offline), Tamagui `<Gate>`/`<UpgradePrompt>` → mobile paywall route. **Migrated `apps/mobile/src/billing/types.ts`** to re-export the canonical 9-entry `@binderly/entitlements` `PaidFeature` union (was a local 8-entry union; renames `pricing_graphs`→`pricing_history`, `export_csv`→`export_data`, `remove_branding`→`shareable_themes`, `cloud_ai_scan_fallback`→`cloud_ai_scan`, adds `save_smart_collections`). **Gate wired** at the one un-gated site on each platform: custom-collections create (web `CustomCollectionsView`, mobile `CustomCollectionsScreen`) replaced its hardcoded free-3-cap-for-everyone with entitlement-aware `useLimitGate('customCollections', count)` (Pro unlimited; free still capped at 3; fail-closed so a pending read never over-grants past the cap), + gate-presence regression tests on both. Other enumerated pro sites — smart-collection save + public shareables on both platforms — were **already tier-gated by their owning feature tasks**; left as-is + documented (re-wiring exceeds the add-a-gate-check boundary). Shareable-themes (T-SH-THEMES) + grading-prediction (T-GR-COMMUNITY-FLYWHEEL) are sibling-owned and self-wire via the exported hook; export/pricing-history/cloud-AI have no UI yet (gates reserved). **Suites:** feature-flags 45, web 790, mobile 1631 (full suite post-rebase); lint/typecheck/build green locally. Raised **#FU-57** (continuous stack-scanner gate — needs the free single-card-scan mode built first). No Q raised) — **iter 34**
