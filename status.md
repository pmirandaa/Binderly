...
**Stage 08 shareables polish:** **1 / 3 merged** (T-SH-CONFIG-MODEL — iter 32; ships owner-side share config + settings UI on web (`/settings/shareables`) + mobile (`/settings/shareables` Expo Router route). Q-020 escalated + closed in same iter — the dispatch brief's `user_share_config` proposed schema collapsed onto the already-merged `profile` (handle, displayName, bio) + `shareable` (slug, theme, show toggles) tables; settings UI now drives both via the existing `profile` and `shareables` api-client resources + a new `checkHandleAvailability()` method on `ProfileResource` with client-side `shareableHandleSchema` pre-validation. UI degrades gracefully when the backend `/v1/me/handle-available` endpoint isn't deployed yet — surface shows "we'll verify on save" copy; #FU-52 logs the matching backend route. Kill-switch (`is_active`) + `social_links_json` are deferred to follow-ups #FU-50 + #FU-51 since they need DB schema additions outside the narrowed scope of this task.
...
- T-GR-SURFACE — `2ddaf73` (Stage 07 grading **8/10**; ships `apps/api-python/grading/surface/` (SurfaceModel numpy-linear placeholder + MobileNetV3-small 2-stream avg-pool upgrade path behind `SURFACE_USE_TORCH=1`; frontFull+backFull 2-shot input; optional raking-light 3rd slot for #FU-31; SurfaceMergedDataLoader reads `subgrades->>'surface'`; aggregate = avg of per-shot scores; train/eval/export/infer Makefile) + TypeScript `SurfaceService` factory (`not_implemented` default, `isSurfaceError` guard, `SurfaceRequest` with optional `rakingLightUri`); **82 pytest tests** + **26 mobile vitest tests**; raised #FU-44 (ml_common subgrade_score rename) / #FU-45 (real training) / #FU-46 (wire to endpoint)) — **iter 30**
