# Build status — Iter 28 closed. **Scanner stage CLOSED (6/6).** Stage 07 grading **5/10** (3 data scrapers shipped in parallel; ML training trio T-GR-CORNERS/EDGES/SURFACE is now unblocked). Q-014 deferred to #FU-29.
...
Iter 28 closed three sibling grading data scrapers in parallel — T-GR-DATA-PSA (PSA cert lookup with synthetic HTML fixtures including a Cloudflare block path; writes to existing `grading_training_sample`), T-GR-DATA-EBAY (Finding API `findCompletedItems` with eBay's category 183454; new migration `0021_ebay_graded_listing_observation`), T-GR-DATA-AUCTIONS (PWCC + Goldin shared parser utilities + per-house adapters; new migration `0022_auction_lot_observation`). All three ship mock-by-default with env-flag-gated live mode and rate-limited polite fetchers. **ML training trio (T-GR-CORNERS + T-GR-EDGES + T-GR-SURFACE) is now fully unblocked.** Worker sibling-pair sequencing surfaced exactly the expected friction at the boundaries: (a) the auction migration originally claimed `0021` and was renamed to `0022` after rebase against EBAY's earlier claim; (b) all three workers independently claimed `#FU-37` (and two claimed `#FU-38`) for similar-shaped follow-ups — reconciled below into a single clean numbering.
...
- T-GR-CORNERS — `7c14cc2` (Stage 07 grading **6/10**; pattern-establisher for ML sub-grade trio; ships `grading/ml_common/` shared infrastructure (LabelledGradingSample normaliser for 3 scraper sources, ImageLoader mock-by-default, TrainingLoop numpy-SGD, eval_metrics MAE/RMSE/±1-grade-accuracy, ONNX Gemm+Clip export + onnxruntime round-trip, ConfidenceBand confidence helper; 57 pytest tests) + `grading/corners/` (CornersModel numpy-linear placeholder + MobileNetV3-small upgrade path documented; CornersDataset 4-patch feature builder; train/eval/export/infer entry points; CornersInferenceEngine wrapping onnxruntime; Makefile; 60 pytest tests) + TypeScript `CornersService` factory (not_implemented default, injectable mock, isCornersError guard; 19 vitest tests); onnx+onnxruntime added to dev extras; torch+torchvision documented in new ml extras; raised #FU-42 real CNN training / #FU-43 on-device inference) — iter 30
- T-GR-DATA-AUCTIONS — `50304f4` (Stage 07 grading **5/10**; PWCC + Goldin auction archive scrapers at `apps/api-python/grading/scrapers/auctions/`; mock-by-default (AUCTIONS_LIVE=1 gate); 5 HTML fixtures per site (PSA-10, BGS-9.5+subgrades, CGC, raw-excluded, multi-lot); migration 0022 `auction_lot_observation` with hammer price + premium + parsed grade + images + SHA-256 dedup; shared _shared/ utilities: grade_parser (PSA/BGS/CGC/SGC + Black Label sub-grades), currency parser (→ cents + ISO-4217), rate-limited httpx client; job.py end-to-end runner; dedup: UNIQUE(auction_house, lot_id) + content hash; 116 pytest tests; raised #FU-37 robots.txt verify / #FU-38 JS-rendering / #FU-39 printing_id fuzzy-match) — iter 28
- T-GR-DATA-EBAY — `8160ea3` (Stage 07 grading **4/10**; eBay sold-listings scraper at `apps/api-python/grading/scrapers/ebay/`; Finding API `findCompletedItems` with `SoldItemsOnly=true`; mock-by-default (EBAY_GRADING_LIVE=1 gate); 7 fixture JSON files (PSA-10/USD, BGS-9.5+subgrades/USD, CGC-8/USD, SGC-9/USD, unknown-grader/OTHER, misgraded-title, BGS-9.5/GBP); migration 0021 `ebay_graded_listing_observation` with pricing intelligence columns + Drizzle schema; title regex parser (company priority order, grade adjacency confidence penalty, BGS sub-grade extraction); dedup: UNIQUE(listing_id) + run-level set; secondary upsert to `grading_training_sample` (source='ebay_sold'); 49 pytest tests; raised #FU-37 image ingest / #FU-38 printing_id fuzzy-match) — iter 28
- T-GR-DATA-PSA — `6445405` (Stage 07 grading **3/10**; PSA cert-lookup scraper at `apps/api-python/grading/scrapers/psa/`; mock-by-default (PSA_LIVE=1 gate); 8 synthetic HTML fixtures (PSA 10/9/8/1, Authentic, Authentic Altered, OC-qualifier, Cloudflare block); writes to existing `grading_training_sample` (source='psa_cert') — no new migration; `httpx`+`bs4`+`lxml`; rate-limit enforced with clock mock; SHA-256 dedup; TosBlockError halts batch; 72 pytest tests; raised #FU-37 browser-seam validation / #FU-38 printing_id fuzzy-match) — iter 28
- T-GR-CENTERING — `65f15a2` (Stage 07 grading **2/10**; Python `grading/centering/` PSA centering measurement: multi-pass Canny outer-edge detector + threshold-based inner-border finder + margin→ratio→grade-hint mapping (55/45→10, 60/40→9, 65/35→8, 70/30→7); 53 pytest tests; TypeScript `src/grading/centering/` typed contract + session-store seam (**closes #FU-32**); `CenteringService` factory pattern (v1: `not_implemented` stub); `useCentering` hook + `CenteringScreen`; +30 mobile tests; adds `opencv-python-headless` to api-python; raised #FU-36 pure-JS on-device centering) — iter 27
