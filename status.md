# Build status — Iter 30 in progress. **Scanner stage CLOSED (6/6).** Stage 07 grading **8/10** (T-GR-EDGES + T-GR-SURFACE merged). **Stage 09 offline-sync (1/3)**. Q-44 raised (ml_common generalisation).
...
**Stage 07 grading:** **8 / 10 merged** (T-GR-CAPTURE-UX #79 — iter 26; T-GR-CENTERING #81 — iter 27; T-GR-DATA-PSA #82, T-GR-DATA-EBAY #83, T-GR-DATA-AUCTIONS #84 — iter 28; T-GR-CORNERS #86 — iter 29; T-GR-EDGES #87, T-GR-SURFACE #88 — iter 30). Capture + centering + 3 data scrapers + corners + edges + surface sub-grade models + `ml_common/` shared training infra shipped. T-GR-CENTERING + T-GR-AGGREGATE remaining.
**Stage 08 shareables polish:** 0 / 3 merged.
**Stage 09 offline-sync:** **1 / 3 merged** (T-OF-LOCAL-DB #85 — iter 29). Foundation (local DB + 3 repositories + subscribe-hook seam) live; T-OF-QUEUE + T-OF-CONFLICTS pending.
...
- T-GR-CORNERS — `7c14cc2` (Stage 07 grading 6/10; pattern-establisher; ships `ml_common/` shared infrastructure; CornersModel + CornersService; +117 Python tests + 19 mobile tests; raised #FU-42 / #FU-43) — iter 29
...
44. **#FU-44 — Rename `corners_score` to `subgrade_score` in `LabelledGradingSample` (ml_common generalisation; T-GR-SURFACE follow-up; iter 30).** All three sub-grade modules (corners / edges / surface) re-use `corners_score` as a generic label slot since it is the only numeric target field in the shared type. The naming mismatch is confusing and error-prone. A coordinated rename in `ml_common/types.py` + all three `dataset.py` files (corners / edges / surface) can land once all three sub-grade tasks are merged. Blocked on: T-GR-EDGES merged (already merged in iter 30); T-GR-SURFACE merged (merged iter 30 PR #88). Both siblings affected. **Logged as #FU-44.**
45. **#FU-45 — Real training pass for surface sub-grade model (T-GR-SURFACE follow-up; iter 30).** The `SurfaceModel` CI implementation is a numpy linear regression placeholder. Real training requires: (a) enough labelled PSA cert / eBay / auction rows with non-null `surface` sub-grade; (b) `torch + torchvision` installed (`pip install -e ".[ml]"`); (c) running `make train SURFACE_USE_TORCH=1` in `apps/api-python/grading/surface/`; (d) exporting the ONNX artifact to R2 at `models/grading/surface/v1/model.onnx`. Note: raking-light shot (better surface accuracy) is not yet captured — see #FU-31. Blocked on: labelled data (depends on #FU-40 printing_id match + community flywheel launch). **Logged as #FU-45.**
46. **#FU-46 — Wire `SurfaceService` to the deployed Python grading endpoint (T-GR-SURFACE follow-up; iter 30).** The TypeScript `SurfaceService` v1 returns `not_implemented`. A future impl makes a network call to the Python grading service once T-GR-SERVING deploys the FastAPI endpoint. Blocked on: T-GR-SERVING (not yet dispatched); #FU-45 (trained model). **Logged as #FU-46.**
