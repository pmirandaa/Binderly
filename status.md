# Build status — Iter 29 closed. **Scanner stage CLOSED (6/6).** Stage 07 grading **6/10** (T-GR-CORNERS shipped + established `ml_common/` shared infra; EDGES + SURFACE unblocked for parallel iter 30). **Stage 09 offline-sync opened (1/3)** with T-OF-LOCAL-DB. Q-014 deferred to #FU-29.
...
**Stage 07 grading:** **6 / 10 merged** (T-GR-CAPTURE-UX #79 — iter 26; T-GR-CENTERING #81 — iter 27; T-GR-DATA-PSA #82, T-GR-DATA-EBAY #83, T-GR-DATA-AUCTIONS #84 — iter 28; T-GR-CORNERS #86 — iter 29). Capture + centering + 3 data scrapers + corners sub-grade + `ml_common/` shared training infra shipped. T-GR-EDGES + T-GR-SURFACE now copy the established pattern.
...
- T-OF-LOCAL-DB — `3005b13` (**Stage 09 offline-sync opened 1/3**; mobile SQLite mirror at `apps/mobile/src/db/` + 3 typed repositories at `apps/mobile/src/repositories/` (UserCollection / CustomCollection / SmartCollection); 6 tables (user_collection_item, custom_collection, custom_collection_item, smart_collection, printing_lite, _meta); `onLocalWrite(observer)` event-emitter for T-OF-QUEUE consumption; `LocalWriteEvent<T>` + `sync_status` ('synced' | 'pending_create' | 'pending_update' | 'pending_delete'); expo-sqlite tested via `vi.mock('expo-sqlite')` backed by sql.js WASM in `test-utils/setup.ts`; +104 mobile tests → 1099; raised #FU-41 full-catalog mirror; raised Q-016 set.logo_url caching) — **iter 29 cap**
- T-GR-CORNERS — `7c14cc2` (Stage 07 grading **6/10**; pattern-establisher for ML sub-grade trio; ships `apps/api-python/grading/ml_common/` shared infrastructure (LabelledGradingSample normaliser over 3 scraper sources, ImageLoader mock-by-default with `CORNERS_LIVE_IMAGES=1` gate, TrainingLoop numpy-SGD, eval_metrics MAE/RMSE/±1-grade-accuracy/confusion matrix, ONNX Gemm+Clip export + onnxruntime round-trip within atol=1e-4, ConfidenceBand confidence helper) + `apps/api-python/grading/corners/` (CornersModel numpy-linear placeholder for CI smoke + MobileNetV3-small upgrade path documented behind `CORNERS_USE_TORCH=1`; 4-patch feature builder; train/eval/export/infer entry points; Makefile) + TypeScript `CornersService` factory (`not_implemented` default, injectable mock, `isCornersError` guard) mirroring CenteringService shape; +117 Python tests (1 skipped at live-image gate) + 19 mobile tests; onnx + onnxruntime added to dev extras; torch+torchvision documented in new `[ml]` extras; raised #FU-42 real CNN training / #FU-43 on-device ONNX inference) — iter 29
- T-GR-DATA-AUCTIONS — `50304f4` (Stage 07 grading 5/10; PWCC + Goldin auction archive scrapers; migration 0022 `auction_lot_observation`; shared `_shared/` parser utilities; 5 HTML fixtures per site; 116 pytest tests; raised #FU-37 / #FU-38 / #FU-39) — iter 28
- T-GR-DATA-EBAY — `8160ea3` (Stage 07 grading 4/10; eBay Finding API `findCompletedItems` scraper for graded slabs; migration 0021 `ebay_graded_listing_observation`; 7 fixture JSONs; 49 pytest tests) — iter 28
- T-GR-DATA-PSA — `6445405` (Stage 07 grading 3/10; PSA cert-lookup scraper writing to existing `grading_training_sample` with `source='psa_cert'` — no new migration; 8 synthetic HTML fixtures incl. Cloudflare-block path; 72 pytest tests) — iter 28
