...
- T-DL-RLS-PG-STAT-REVOKE — `c61eed8` (migration `0020_revoke_admin_debug_view_grants.sql` revokes anon + authenticated grants on `v_pg_stat_statements_top_queries`; `verify-rls` now reports 0 failures across the canonical surface; one-line REVOKE per #FU-28) — iter 24
...
34. **Cosmetic chore for Pablo: orphan worktree dirs at `/Users/pmiranda/Stuff/binderly-wt-T-*`.** Git no longer tracks them as worktrees; safe to `rm -rf`. pnpm-store residue blocks sandbox `rm`. Affected: T-DL-SEED-INGEST, T-FN-LINT-CONFIG, T-M-COLLECTION, T-W-BROWSE, T-W-COLLECTION, T-W-CUSTOM, T-W-SHAREABLE-PUBLIC, T-W-SMART, T-BE-EDGE-FUNCTIONS-V2, T-W-API-V2-WIRING, T-M-API-V2-WIRING, T-SC-CAMERA, T-SC-EMBED-MODEL, T-BE-Q013-CLEANUP, T-DL-RLS-PG-STAT-REVOKE, T-SC-ANN-INDEX, T-SC-DETECT, T-SC-MATCH, T-GR-CAPTURE-UX.
