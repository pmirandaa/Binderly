...
- T-GR-EDGES — `2f47ef8` (Stage 07 grading **7/10**; ships `apps/api-python/grading/edges/` (EdgesModel numpy-linear placeholder for CI smoke + MobileNetV3-small 4-strip shared trunk upgrade path behind `EDGES_USE_TORCH=1`; 4 perimeter strips at 32 px thickness on 256×256 normalised image; EdgesMergedDataLoader + EdgesLabelledSample parallel data layer due to ml_common corners coupling; train/eval/export/infer entry points; Makefile) + TypeScript `EdgesService` factory (`not_implemented` default, `isEdgesError` guard, `EdgesRequest` with 4 strip URIs tuple); **84 pytest tests** + **22 mobile vitest tests**; raised Q-017 (ml_common generalisation for multi-subgrade support — closed in same iter, resolved by #FU-44 omnibus); raised #FU-47 HPO + dataset curation across sub-grades; EDGES's parallel #FU-45/46 claims (torch CNN, on-device ONNX) folded into SURFACE's #FU-45/46 omnibus framing at iter-close) — **iter 30**
