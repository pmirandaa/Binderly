# Build status — **Iter 32 in flight.** **Scanner stage CLOSED (6/6).** Stage 07 grading **9/10** (T-GR-AGGREGATE merged). **Stage 08 shareables polish 1/3** (T-SH-CONFIG-MODEL merged — owner-side share config + settings UI on web + mobile, mapped onto existing profile + shareable tables; backend handle-availability endpoint deferred to follow-up #FU-52). **Stage 09 offline-sync CLOSED (3/3)** (T-OF-CONFLICTS merged). Q-016 resolved (T-OF-QUEUE; set_logo_url denormalised); Q-017 raised + closed in iter 30 (resolved by #FU-44). Q-018 raised + closed in iter 31 (T-GR-AGGREGATE; confidence-band categorical thresholds + centering grade-hint mapping documented). Q-019 raised + closed in iter 31 (T-OF-CONFLICTS; api-client single-GET gap + repository upsertFromServer non-API both worked around via in-module adapters; #FU-49 logs the api-client cleanup). Q-020 raised + closed in iter 32 (T-SH-CONFIG-MODEL; dispatch brief's `user_share_config` model collapsed onto already-merged `profile` + `shareable` tables — narrowed-scope plan logged in the elaborated task brief; #FU-50/#FU-51/#FU-52 cover the kill-switch, social-links, and handle-availability backend follow-ups).
...
**Stage 07 grading:** **9 / 10 merged** (T-GR-CAPTURE-UX #79 — iter 26; T-GR-CENTERING #81 — iter 27; T-GR-DATA-PSA #82, T-GR-DATA-EBAY #83, T-GR-DATA-AUCTIONS #84 — iter 28; T-GR-CORNERS #86 — iter 29; T-GR-EDGES #87, T-GR-SURFACE #88 — iter 30; T-GR-AGGREGATE — iter 31). Capture + centering + 3 data scrapers + 3 sub-grade models + `ml_common/` shared training infra + final-stage PSA-calibrated aggregator with 3-band categorical confidence shipped. Only T-GR-COMMUNITY-FLYWHEEL remaining.
...
**Stage 10 paywall / billing:** 1 / 4 merged (T-PB-REVENUECAT — iter 32).
**Stage 11 deployment:** 0 / 6 merged.
...
- T-PB-REVENUECAT — `<squash-sha>` (**Stage 10 paywall / billing 1 / 4**; ships `apps/mobile/src/billing/` — RevenueCat SDK adapter (`sdk.ts` real + dev-mode stub with `createEmptyCustomerInfo` baseline so feature gates evaluate to `free` tier when `EXPO_PUBLIC_REVENUECAT_IOS_KEY` / `EXPO_PUBLIC_REVENUECAT_ANDROID_KEY` are absent — graceful degradation per T-W-AFFILIATE-LINKS pattern), lazy `initializeBilling()` + `useBillingBootstrap()` React hook that wires to `AuthProvider.useAuth()` and calls `Purchases.configure` once on first authenticated session, then `logIn(userId)` / `logOut()` on subsequent transitions (never at module-import time → acceptance criterion enforced by `init.test.ts`), 4 TanStack-Query hook surfaces (`useOfferings()` / `usePurchasePackage()` / `useEntitlementsQuery()` + `usePaidFeature(feature)` / `useRestorePurchases()`) with strict types re-exported from `react-native-purchases`, `PRO_ENTITLEMENT_ID = 'pro'` contract documented for T-PB-PADDLE webhook grant logic (RC dashboard maps both stores' products to the same entitlement id; T-PB-ENTITLEMENTS will read this as the cross-platform source of truth), 8-feature `PaidFeature` discriminated string union seeded from `rules/10-paywall-billing.md` (`unlimited_custom_collections` / `unlimited_shareables` / `stack_scanner` / `grading_prediction` / `pricing_graphs` / `export_csv` / `remove_branding` / `cloud_ai_scan_fallback` — T-PB-GATING will swap each free-tier limit's compile-time check against this surface so unknown features fail typecheck rather than silently granting), `PurchaseResult` discriminated union mapping RC's native errors to 6 typed variants (`success` / `cancelled` / `payment_pending` / `payment_invalid` / `network_error` / `error`) with full type-guard suite, optimistic entitlements cache update on purchase + restore success so the rest of the app sees `tier === 'pro'` immediately without a server round-trip; `useEntitlementsQuery` does not modify gating screens (deferred to T-PB-GATING per task scoping). **+97 mobile vitest tests (1480 total)** across 7 test files (`init` lazy + idempotent + auth-event-driven; `bootstrap` React-hook wire-up; `sdk` stub adapter + `createEmptyCustomerInfo`; `entitlements` 32 tests covering tier derivation, expired-entitlement filtering, error surfacing, paid-feature gating across all 8 features; `offerings` cache + dev-mode sentinel + error surfacing; `purchase` 22 tests covering 6-variant error mapping + optimistic cache; `restore` success + error + cache). Coordinated with T-PB-PADDLE on `'pro'` entitlement-id string via the rules file. No Q-NN raised (rules file fully specified the v1 feature surface; native config — Xcode entitlements / App Store Connect product ids / Google Play Billing — is downstream deployment work outside this task's scope and tracked by Stage 11 deployment tasks)) — **iter 32**
- T-SH-CONFIG-MODEL — `<squash-sha>` (**Stage 08 shareables polish 1/3**; ships owner-side share config + settings UI on web (`apps/web/app/settings/shareables/`) + mobile (`apps/mobile/src/screens/shareables/` + `apps/mobile/app/settings/shareables.tsx`). Q-020 escalated + closed in same iter — instead of carving the brief's proposed `user_share_config` single table, the worker mapped the logical model onto the already-merged `profile` (handle, displayName, bio) + `shareable` (slug, theme, showValues/showMissing/showPhotos) tables so the existing `client.profile.*` + `client.shareables.*` resources drive the new UI without a destructive schema migration. Added `checkHandleAvailability(handle)` to `ProfileResource` + `handleAvailabilityResponse` DTO + `SHAREABLE_HANDLE_PATTERN` / `shareableHandleSchema` to `@binderly/api-contracts` (single source of truth shared by `apps/web/app/settings/shareables/validation.ts` + `apps/mobile/src/screens/shareables/validation.ts`); client method pre-validates via the shared zod schema before issuing `GET /v1/me/handle-available`, and the settings UI degrades to "we'll verify on save" copy when the backend route 404s (deferred to #FU-52). Free-tier shareable cap (1 shareable per PROJECT.md § 16) enforced in the settings view with a `cap-badge` testID + disabled create button; non-default themes + `showValues` toggle locked behind `subscription.tier === 'pro'` with inline "(Pro)" tags. Optimistic updates + rollback on API error for both profile + per-shareable rows; two-step inline delete confirmation. **+44 mobile vitest tests** (19 validation, 21 screen, 4 route) **+40 web vitest tests** (validation + view + route). Raised **#FU-50** (`T-SH-KILL-SWITCH`: `is_active` column on `shareable` + RLS read-path gate on public Edge fn `/v1/c/{handle}/{slug}`) + **#FU-51** (`T-SH-SOCIAL-LINKS`: `social_links_json` JSONB on `profile` + tag-style editor UI) + **#FU-52** (`T-BE-SHAREABLES-HANDLE-CHECK`: `GET /v1/me/handle-available?handle=…` Edge fn with `available`/`reason: taken|invalid|reserved|rate_limited` response shape matching `handleAvailabilityResponse` DTO already shipped) — **iter 32**
- T-SH-OG-IMAGES — `a37b82a` (**Stage 08 shareables 1/3**; ships `/api/og/share/[handle]/[slug]` Next.js Route Handler at `apps/web/app/api/og/` rendering a 1200×630 PNG via `next/og` `ImageResponse` (gradient background from `packages/ui` brand palette + top-left wordmark + center-large collection title + lower-third 4-tile thumbnail row + footer URL/stat line); `apps/web/lib/og/` helpers (pure render + fallback + 2 s-per-image fetcher with brand-coloured silhouette fallback + 2.5 s outer payload-fetch timeout + cache-headers + brand tokens + base64-encoded SVG silhouette); cache `public, max-age=3600, s-maxage=86400, stale-while-revalidate=604800` on success / `public, max-age=3600` on fallback; unknown handle / `is_public=false` / network failure / Satori throw all degrade to a generic Binderly-branded fallback PNG (NOT 404 — social platforms cache 404s aggressively); **85 vitest tests** added across 7 suites (584/584 web suite green); minimal one-line metadata wire-up on `apps/web/app/c/[handle]/[slug]/page.tsx` (`metadata.openGraph.images` + `metadata.twitter.images` now point at the new route; sibling file-based `opengraph-image.tsx` stays in place as passive secondary fallback); no FUs / Q-NN raised) — **iter 32**
- T-OF-CONFLICTS — `<squash-sha>` (**Stage 09 offline-sync CLOSED 3/3**; ships `apps/mobile/src/sync/conflicts/` LWW resolver consuming T-OF-QUEUE's `onDeadLetter` seam — `ConflictResolver.resolve(event)` decodes the queue payload, fetches fresh server state via `ServerFetcher` adapter (single-GET for `custom_collection` / `smart_collection_rule`; client-side pagination walk of `listCollectionItems` / `listCustomCollectionItems` for the 2 entity types api-client lacks single-GETs for, capped at 50 pages → `transient_error` if exceeded), compares `updated_at` (LWW; ties → server wins; unparseable → server wins; `Date.parse` on both timestamps before any compare so lexicographic UTC fallback isn't fooled by garbage strings), and applies one of 4 resolutions: `local_won` (re-enqueues original mutation + deletes dead-letter row), `server_won` (direct-SQL `INSERT OR REPLACE` via `local-writer.ts` that bypasses repository `onLocalWrite` → no feedback loop), `server_won_deleted` (404 + local op was update/delete OR server delete after local create→update sequence; direct-SQL `DELETE` + emits `ConflictResolvedEvent` for the future toast UX), `transient_error` (API 5xx / network → dead-letter row left in place + logged for next connectivity retry); all resolution paths wrapped in a single `sqliteTransaction()` for crash-safety + idempotency. v3 SQLite migration adds the append-only `sync_conflict_log` table (id PK / table_name / entity_id / op_type / resolution / local_payload_json / server_payload_json / local_updated_at / server_updated_at / error_detail / created_at) with table + entity-name index for retrieval by ConflictDebug screen later. `startConflictResolver({ syncQueueRepo, apiClient, repositories, conflictLogRepo, onResolved? })` wiring shim mirrors T-OF-QUEUE's `startSync()` shape, returning an unsubscribe handle; `drainPendingDeadLetters()` for cold-start replay after app launch. **+161 mobile vitest tests (1359 total)** across 9 test files (comparator / event-emitter / migration-v3 / log-repository / server-fetcher / local-writer / resolver / start + shared _helpers); raised Q-019 (api-client single-GET gap + repository upsertFromServer non-API; both worked around via in-module adapters — closed by documented assumptions) + #FU-49 (`T-BE-API-CLIENT-SINGLE-GET-COLLECTION-ITEMS`; add `getCollectionItem(id)` + `getCustomCollectionItem({ customCollectionId, printingId })` to drop the page-walk + `MAX_PAGE_WALK` cap; small additive task)) — **iter 31 cap**
- T-GR-AGGREGATE — `b97d7d8` (Stage 07 grading **9/10**; ships `apps/api-python/grading/aggregate/` — `LinearWeightedAggregator` numpy linear-weighted-sum (priors centering 0.25 / corners 0.35 / edges 0.25 / surface 0.15; constructor-configurable; AGGREGATE_USE_TORCH=1 learned-calibration upgrade path docstring), PSA 0.5-tick rounding with banker's rounding for `.25`/`.75` determinism, 3-band `'low'|'medium'|'high'` categorical mapped from float ConfidenceBand.confidence at 0.66/0.33 thresholds plus a score-spread heuristic (`< 1.0` for 'high', `>= 2.0` forces 'low'), `aggregate_subgrades()` pure workhorse + `infer()` adapter normalising `CenteringResult.grade_hint` string enum to float score, ONNX export round-trip via shared `ml_common.model_export`, MAE/RMSE/±1 eval against PSA cert rows, Makefile + README; **+112 pytest tests** (target 40+; runs without torch) + **+24 mobile vitest tests** (target 20+); TypeScript `AggregateService` factory mirrors corners/edges/surface (`not_implemented` default, `isAggregateError` guard, `AggregateRequest` consuming the 4 upstream prediction objects); raised Q-018 (confidence-band categorical thresholds + centering adapter — closed by documented assumptions, non-blocking) and #FU-48 (learned-weights upgrade once labelled grading sessions land)) — **iter 31**
