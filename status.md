# Build status — **Iter 38 closed. 🎉 86/86 formal tasks merged + entire UNBLOCKED follow-up backlog cleared.** All 11 stages CLOSED; migrations through `0028`; web/mobile/api/python all build + lint + typecheck green. This iter shipped the last unblocked FUs: card-detail price trends (#FU-66, Q-028 resolved), OG hero theme palette (#FU-62), free single-card scan + Pro stack gate (#FU-57), capture→centering router-params (#FU-32), and `/sets/*` slug convention (#FU-64 /sets half). **Everything still open is BLOCKED on external inputs and now needs Pablo:** (1) labelled grading dataset → real CNN/calibration training (#FU-30/36/42/43/45/47/48/60); (2) data-collection budget + scraper API creds → live-fetch + graded-image ingest (#FU-37/38/39); (3) production secrets → Stage-11 go-live (#FU-53), incl. the Q-021 Python FastAPI entrypoint for Fly; (4) Xcode + Android Studio → native ANN SIMD module (#FU-29); (5) PRODUCT DECISIONS: Q-022 (mobile OTA cadence), Q-026 (`/cards/*` printing-vs-card entity → #FU-64 remainder), Q-027 (per-binder social links), Q-030 (scanner mode-toggle vs rules/06 'no toggle'), Q-031 (getSetBySlug scan vs PostgREST filter → #FU-69), #FU-68 (freshness-band re-derivation). Autonomous burndown complete — no further unblocked work remains.
...
**Stage 08 shareables polish:** ✅ **CLOSED — 3 / 3 merged** (T-SH-OG-IMAGES — iter 32; T-SH-CONFIG-MODEL — iter 32; T-SH-THEMES — iter 34). **Owner-config follow-ups #FU-50/#FU-51/#FU-52 merged — `5f097a6`, #111** (kill-switch + social-links + handle-check; migration **0027**). T-SH-OG-IMAGES ships the dynamic OG image route at `/api/og/share/[handle]/[slug]` (1200×630 PNG hero; gradient background; top-left wordmark; centre title; 4-tile lower row from member thumbnails with 2 s per-image timeout + brand-coloured silhouette fallback; footer URL + stat line; `Cache-Control: public, max-age=3600, s-maxage=86400, stale-while-revalidate=604800` on success, `public, max-age=3600` on fallback; unknown handle / `is_public=false` / network failure all serve a generic Binderly-branded fallback PNG instead of 404 since social platforms cache 404s; public page `generateMetadata` now wires `<meta property="og:image">` + Twitter card images at the new route). T-SH-CONFIG-MODEL ships the owner-side share config + settings UI on web (`/settings/shareables`) + mobile (`/settings/shareables` Expo Router route) mapped onto the already-merged `profile` (handle, displayName, bio) + `shareable` (slug, theme, show toggles) tables; Q-020 escalated + closed in same iter — dispatch brief's `user_share_config` proposed schema collapsed onto the existing two-table physical model via new `checkHandleAvailability()` on `ProfileResource` + `shareableHandleSchema` in `@binderly/api-contracts`; UI degrades gracefully when the backend `/v1/me/handle-available` endpoint isn't deployed yet — surface shows "we'll verify on save" copy (#FU-52 logs the matching backend route). Kill-switch (`is_active`) + `social_links_json` deferred to follow-ups #FU-50 + #FU-51 since they need DB schema additions outside the narrowed scope of this task. **T-SH-THEMES** closes the stage: a typed registry of 5 flat themes (`default` free; `dark`/`paper`/`neon`/`gold` Pro) in `apps/web/app/c/themes/` pinned to the `SHAREABLE_THEMES` enum, `resolveTheme` (stale/unknown/null → default) + `resolvePublicTheme(id, ownerIsPro)` (free-tier downgrade), an SSR-safe `<ShareableThemeProvider>` the public page wraps content in, and a pro-gated `<ThemePicker>` gallery in settings consuming the merged `useGate('shareable_themes')` hook (T-PB-GATING). `ogPaletteForTheme` exports a flat Satori-friendly palette for a later OG-theming wire-up. Q-024 raised (public payload lacks owner tier → server-side free-downgrade render can't enforce yet; interim renders the stored theme for everyone, downgrade branch pre-wired) → #FU-61 (T-BE-SHAREABLE-OWNER-TIER, ✅ CLOSED `43664d5` #113 — owner tier now on the public payload + free-theme downgrade enforced server-side; Q-024 resolved); OG-theming wire-up → #FU-62 (T-SH-OG-THEME-WIRE). T-SH-THEMES remaining.
**Stage 10 paywall / billing:** ✅ **CLOSED — 4 / 4 merged** (T-PB-REVENUECAT + T-PB-PADDLE — iter 32; T-PB-ENTITLEMENTS — iter 33; T-PB-GATING — iter 34). **T-PB-GATING** closes the functional side of Stage 10: ships the new framework-agnostic `@binderly/feature-flags` package (pure `evaluateGate(entitlement, feature)` + `evaluateLimitGate(entitlement, resource, count)` decision core built on `@binderly/entitlements`' `canUseFeature`/`withinFreeLimit`; `GateResult = {allowed:true} | {allowed:false, reason:'requires_pro'|'free_limit_reached', feature, limit?}`; **45 pure tests**) + web glue (`apps/web/lib/gating/`: `useGate`/`useLimitGate` over `getMyEntitlements()`, `<Gate>`/`<UpgradePrompt>` → `/billing`, fail-closed on loading/error → free) + mobile glue (`apps/mobile/src/lib/gating/`: same hooks reading RC directly via `useEntitlementsQuery`, Tamagui `<Gate>`/`<UpgradePrompt>` → mobile paywall). **Migrated `apps/mobile/src/billing/`'s 8-entry `PaidFeature` union to re-export the canonical 9-entry `@binderly/entitlements` union** (renames threaded through). **Gates wired:** custom-collections create cap (web `CustomCollectionsView` + mobile `CustomCollectionsScreen`) flipped from a hardcoded free-3-for-everyone cap to entitlement-aware `useLimitGate` (Pro = unlimited) + gate-presence regression tests on both. **Already-gated sites left as-is + documented:** smart-collection save (web `SmartEditorView` + mobile `SmartCollectionEditorScreen`) and public shareables (web + mobile settings) already gate on `subscription.tier` via their owning feature tasks — re-wiring risks breaking their suites + exceeds the add-a-gate-check boundary, so left untouched. **Left for siblings:** shareable-themes (T-SH-THEMES) + grading-prediction/community (T-GR-COMMUNITY-FLYWHEEL) self-wire via the exported hook (their owns_paths not touched). **No web/mobile UI exists yet** for export (CSV/JSON), pricing-history, or cloud-AI-scan — gates reserved in the union, no site to wire. **Continuous stack-scanner gate has no clean seam** (the free single-card-scan mode isn't built; gating it is a feature, not a gate-check) → **#FU-57**. Suites: feature-flags **45**, web **790** (incl. new web-glue + custom-collections gate tests), mobile **1631** (full suite post-rebase incl. siblings; incl. new mobile-glue + custom-collections gate tests); lint / typecheck / build all green locally (Node 22.13.0; Actions skipped per quota policy). No Q raised (the downgrade-on-cancel question resolved with the documented default: never destroy data — gate *new* over-limit creation, leave existing read-only). **T-PB-ENTITLEMENTS** ships the keystone unified entitlement service making RevenueCat the canonical *read* path: new `@binderly/entitlements` package (`model.ts` canonical `Tier` + 9-entry `PaidFeature` union from PROJECT.md § 16 + `FREE_LIMITS` {customCollections:3, shareables:1} + `PRO_ENTITLEMENT_ID='pro'`; `can.ts` pure `canUseFeature`/`withinFreeLimit` gate helpers; `revenuecat-client.ts` resilient fail-closed RC REST read over `GET /v1/subscribers/{id}` — any network/non-200/malformed/missing-key error → `{tier:'free', source:'fallback'}`, never throws) + `GET /v1/me/entitlements` Edge endpoint (slotted into the existing single `v1` mux as `_shared/handlers/entitlements.ts` + route, NOT the stub's `functions/entitlements/` dir — path divergence noted; reads authed user id as RC `app_user_id`, env-missing `REVENUECAT_SECRET_API_KEY` → free fallback + 200 + logged warn, never 500) + `entitlements` resource (`getMyEntitlements()`) on api-client + `entitlementsDto` on api-contracts. RC read logic is **mirrored** in the edge handler (Deno bundle can't import workspace pkgs — same constraint as `_shared/contracts.ts`), pinned to § 16 by a parity test. **Canonical `PaidFeature` reconciliation:** spec matrix wins over mobile's 8-entry union (renames: `pricing_graphs`→`pricing_history`, `export_csv`→`export_data`, `remove_branding`→`shareable_themes`, `cloud_ai_scan_fallback`→`cloud_ai_scan`; adds `save_smart_collections`); migrating `apps/mobile/src/billing/` to import the hoisted union is T-PB-GATING's job. **+104 tests** (73 `@binderly/entitlements` ≥40 target; +8 api-contracts → 258; +7 api-client → 270; +16 edge-fn → 329). lint / typecheck / build green locally. No Q raised (RC GET-subscriber lazily returns empty subscriber → new users read free, no creation step / product decision). Raised #FU-53 (entitlement-fallback observability). T-PB-ENTITLEMENTS unblocks T-PB-GATING + T-SH-THEMES + T-GR-COMMUNITY-FLYWHEEL. T-PB-PADDLE ships Paddle MoR (Merchant of Record) checkout shipped on web at `/billing` with auth-gated, env-degrading UI (free → 2 plan cards monthly + annual / pro → manage-via-Paddle stub) backed by `@paddle/paddle-js@1.6.4` overlay; HMAC-SHA256 webhook verifier at `/api/paddle/webhook` (5-minute timestamp tolerance, `node:crypto` `timingSafeEqual`) maps 7 Paddle event types (`subscription.created|activated|canceled|past_due|paused|resumed`, `transaction.completed`) to RevenueCat REST grants/revokes (server-side `forwardEntitlement` against `/v1/subscribers/:id/entitlements/pro/promotional`); 2 SQL migrations land `paddle_webhook_log` audit table (event id unique for idempotency, action ∈ {grant, revoke, ignore}, retry flag, jsonb payload) under service-role-only RLS (verify-rls 131/0 incl. `paddle_webhook_log` × 3 new assertions). Webhook returns 401 on invalid signature, 503 on missing `PADDLE_WEBHOOK_SECRET`, 200 + retry-flag on RC downstream failures (Paddle's at-least-once retries don't re-grant; the audit row is the reconciliation seam). Coordinated `'pro'` entitlement-id with T-PB-REVENUECAT via `apps/web/lib/paddle/plans.ts.PRO_ENTITLEMENT_ID`. **+130 vitest tests** across 11 files (env / plans / signature / events / revenuecat / log / client / entitlements / handler / BillingView / BillingRoute) bringing web suite to 629/629 green; lint + typecheck + build all green locally. T-PB-ENTITLEMENTS + T-PB-GATING remaining.
**Stage 09 offline-sync:** ✅ **CLOSED — 3 / 3 merged** (T-OF-LOCAL-DB #85 — iter 29; T-OF-QUEUE #89 — iter 30; T-OF-CONFLICTS — iter 31). Mutation queue + replay engine live (FIFO `sync_queue` table, idle/replaying/paused state machine, `30 s × 2^attempts` backoff capped at 1 h, 10-attempt dead-letter); LWW conflict resolver consumes the dead-letter seam with per-table server-fetcher adapters (paginated walk fallback for entities the api-client doesn't expose a single-GET for; capped at 50 pages / ~5 000 items before degrading to `transient_error` retry — page-walk later swapped for the #FU-49 single-GETs, cap dropped, #FU-59 `43664d5` #113), v3 `sync_conflict_log` audit table, direct-SQL `local-writer` for server-wins overwrites (bypasses repository `onLocalWrite` to avoid sync-queue feedback loop), and `onConflictResolved` event emitter for the future `T-M-CONFLICT-UX` toast surface (event-emitter contract shipped; UI integration is a screen-layer follow-up out of Stage 09's scope).
**Stage 11 deployment:** ✅ **CLOSED — 6 / 6 scaffolded** (T-DP-VERCEL + T-DP-FLY + T-DP-SUPABASE-PROD + T-DP-R2-PROD — iter 33, shipped as one cluster PR on `agent/T-DP-INFRA`; T-DP-EAS — iter 35, `36da110`; T-DP-MONITORING — iter 35, `92269e7`). **Scaffolding-complete, awaiting Pablo's production secrets** — all four deploy paths ship inert (each live step guarded on its provider secret, skipped not failed) until the secrets enumerated in `infra/DEPLOYMENT_SECRETS.md` are added to the GitHub repo. Vercel deploy (web) guarded on `VERCEL_TOKEN`; Fly deploy (Python svc) guarded on `FLY_API_TOKEN` **and** the Q-021 HTTP-entrypoint follow-up; Supabase migration CI (`deploy-db.yml`) guarded on `PROD_DATABASE_URL` (idempotent forward-only drizzle migrator, no resets); R2 prod buckets + CORS/lifecycle/access-key docs (policy files only, no live keys). EAS build/submit (mobile) guarded on `EXPO_TOKEN` — `apps/mobile/eas.json` (development/preview/production profiles, Expo SDK 52, `appVersionSource: remote`) + `deploy-mobile.yml` (manual-dispatch, opt-in store submit) + `infra/eas/README.md` runbook; a live store submit additionally needs the Apple ASC API key + Google Play service-account JSON. Raised **Q-021** (api-python has no FastAPI/HTTP entrypoint — Fly serving target is batch/CLI-only today) + **Q-022** (mobile release cadence + OTA / EAS Update strategy; #FU-58). **T-DP-MONITORING** (iter 35, `92269e7`, #109) closes the stage: inert-until-secrets observability mirroring the other deploy paths — Sentry (errors) + PostHog (analytics) init seams across web (`apps/web/lib/observability/` + `<ObservabilityInit/>` in the root layout), mobile (`apps/mobile/src/lib/observability/`, called from `app/_layout.tsx`), and python (`apps/api-python/observability/` `init_sentry`, lazy `sentry-sdk`, no-op when absent), all dependency-light via injectable hooks (no provider SDK added yet; go-live wiring in `infra/monitoring/README.md`); web `GET /api/health` liveness route; `deploy-monitoring.yml` Sentry-release CI guarded on `SENTRY_AUTH_TOKEN` (skipped, not failed). Go-live still gated on Pablo's production secrets (#FU-53) + the Q-021 Python HTTP entrypoint; raised Q-025 (deep health-check + pager) → #FU-63.
...
**Local dev-env foundation CLOSED 2026-05-29 (squash `1f3b717`, #112) — 86/86 formal tasks merged; the build graph is complete.** The final four foundation tasks (T-FN-DOCKER → T-FN-SUPABASE-LOCAL → T-FN-DB-MIGRATIONS → T-FN-ENV-CONVENTIONS) are now `merged`. Their deliverables had already shipped incrementally across prior waves — the Compose stack (Postgres 16 / MinIO / Mailpit), the Supabase CLI config (`infra/supabase/config.toml`, 543xx ports, `db:start`/`db:stop`/`db:reset`), the plain-ESM Drizzle migrator (`packages/db/scripts/migrate.mjs`, graceful-degradation journal handling, `db:migrate` / `db:migrate:local`), and the env conventions (`.env.example` 73 vars + `docs/env.md` + `scripts/check-env.sh` + `check:env`). This wave verified completeness, added the missing local-apply runbook to `infra/supabase/README.md`, fixed `.ts`→`.mjs` doc drift in `packages/db/README.md`, and flipped the four statuses to `merged`. No code changes; no migration files touched. Local battery green: lint 14/14, typecheck 24/24, test 884, build full-turbo. Docker-dependent smoke (`pnpm db:start` + `pnpm db:migrate:local`) deferred to a non-sandboxed shell with Docker Desktop running.
...
- T-ROUTING-CLEANUP — `ef3bda6` (#118) (**#FU-32 + #FU-64 (the unblocked `/sets` half) — centering router-params + `/sets/*` slug convention**; LOCAL-CI-only per quota; no DB migrations. **#FU-32**: removed the `@deprecated` module-scoped `__getLastEmittedSession()` capture→centering hand-off (+ its barrel exports); capture now `storeSession()`s the completed `GradingCaptureSession` in the in-memory session-store and pushes a real expo-router param `/grading/centering?sessionId=<id>` (the 4+ stills stay in-memory, only the id rides the URL); the review screen reads it back via `useLocalSearchParams().sessionId`. **#FU-64**: new shared `cards.getSetBySlug` api-client method (bounded `/v1/sets` scan matching the unique `canonical_key`) now backs slug→set on BOTH web + mobile; web `/sets/[id]`→`/sets/[slug]` with legacy-UUID URLs resolving by id + `router.replace`→canonical slug; `BrowseView` links to `/sets/<canonicalKey>`; mobile `useSetBySlugQuery` switched onto the shared method. **`/cards/*` left untouched** (printing-vs-card entity decision still blocked on Q-026). Logged Q-031/#FU-69 (optional move to a PostgREST `canonical_key=eq` filter). Local battery green: lint 14/14, typecheck 24/24, test (mobile 1668, web 889, api-client 285), build 13/13) — **iter 38**
- T-SP-PRICING-DISPLAY-TRENDS + T-SH-OG-THEME-WIRE — `c2ddd33` (#117) (**#FU-66 + #FU-62 — card-detail trends + OG theme palette wiring**; LOCAL-CI-only per quota. Pure consumer-side wires of already-shipped data — no DB migrations. **#FU-66**: web `CardPriceBlock` + mobile `PriceBlock` render the headline 30-day trend arrow + signed % change (server-bucketed `trendDirection` + `trend30dPct`; the ±1% dead-band stays in SQL — no client recompute) plus a "Last seen" line from `lastObservationAt`; new shared pure formatters in `@binderly/pricing-display` (`formatTrendPercent`/`trendArrow`/`trendDirectionLabel`/`hasRenderableTrend`). **#FU-62**: OG hero background-gradient/wordmark/title/tile/footer now read from `ogPaletteForTheme` with the SAME free-tier downgrade as the public page (`resolvePublicTheme` — a free owner's stored Pro theme reverts to the default palette), replacing the fixed brand gradient. **Q-028 resolved** (trend base + ±1% band kept server-side; `freshness` stays on `computedAt`, `lastObservationAt` surfaced additively); #FU-68 logged. Local battery green: lint/typecheck/test (pricing-display 157, full web + mobile suites)/build 13/13) — **iter 37**
- T-GR-CAPTURE-FULL — `84d1123` (#115) (**#FU-31 + #FU-33 — full capture shot-set + live quality gate**; LOCAL-CI-only per quota. **#FU-31** extends the guided capture flow from 4 → 7 shots (adds bottom-left + bottom-right corner crops + the surface raking-light shot per PROJECT.md § 12): the capture state machine, guided-capture steps/overlays, session schema, and the append-only `@binderly/api-contracts` capture model now carry all shots — four corners map to `cornerUrls` in `[TL, TR, BL, BR]` order, surface → `surfaceUrl`; the surface step relaxes its brightness floor (angled raking light runs dark). **#FU-33** wires a live vision-camera frame-processor sample so the capture button reflects real-time quality (sharpness/brightness/coverage) before the user taps, reusing the existing `evaluateCaptureQuality` gate — behind the existing frame-processor abstraction, gracefully no-ops where the native frame processor / `toArrayBuffer` is unavailable (tests) or the pixel buffer ≠ `w·h·3`, throttled to ~10 FPS. Local battery green (mobile 1668, api-contracts 306, typecheck/lint clean, build 13/13); Q-029 raised; #FU-67 logged) — **iter 37**
- T-DL-PRICING-CURRENT-VIEW-V2 — `664c1e0` (#114) (**#FU-5 — pricing-current-view v2**; LOCAL-CI-only per quota. Migration **0028** recreates `mv_current_price` **additively** — all v1 columns preserved + `current_price` (30-day median) + `trend_30d_pct` / `trend_90d_pct` / `trend_all_time_pct` + derived `trend_direction` + `sample_count_30d` + `last_observation_at`; trends calendar-anchored on the slice's own latest `period_start`; grants tightened to SELECT-only (Supabase default-privileges quirk, cf. #FU-28 / `0020`). Contracts (optional trend fields + `priceTrendDirectionSchema` on `currentPriceDto` + `printingCurrentPriceDto`) + the `GET /v1/printings/:id/current-price` handler surface the new fields, fully backward-compatible. **#FU-28 verified MOOT** — `0020` already shipped the `v_pg_stat_statements_top_queries` REVOKE; `verify-rls` 137/0. Local battery green (lint/typecheck/test/build, db 24, edge-fn 369, fresh `db:migrate`); Q-028 raised; #FU-66 logged) — **iter 36**
- T-POLISH-SWEEP — `fc18834` (#110) (**tech-debt / polish sweep — #FU-11 + #FU-14 + #FU-15 + #FU-18 + #FU-20 + #FU-21**; LOCAL-CI-only per quota. **#FU-11** `apps/web` build-with-no-env smoke guard: `build:no-env` script strips every `NEXT_PUBLIC_*`/`SUPABASE_*` var + asserts `next build` exits 0, wired as an explicit CI `build`-job step + a Node-script eslint override. **#FU-14** deleted dead M-SHELL placeholders `SignInScreen.tsx` + `AuthCallbackScreen.tsx` (only the smoke matrix referenced them). **#FU-15** `AuthProvider.tsx` prettier-compliant (formatting only). **#FU-20** new cross-platform `<Modal>` primitive in `@binderly/ui` (backdrop + surface, Esc/backdrop close, dialog a11y, `@tamagui/core`-based) + migrated the 3 web custom-collection consumers onto it + deleted the in-tree `Modal.tsx`. **#FU-21** `@binderly/ui` `<Input>` now forwards `onBlur` / `onEndEditing`. **#FU-18** PARTIAL: `/sets/*` (UUID vs slug — chosen = slug) vs `/cards/*` (web *printing*/`getPrinting` vs mobile *card*/`getCard` — needs product decision); documented + deferred, raised **Q-026**, remainder **#FU-64**. No DB migration; stayed out of shareables. New tests: `@binderly/ui` +10 (Input +4, Modal +6); local lint/typecheck/test/build + `build:no-env` all green (Node 22.13.0). No Q blocked) — **iter 35**
- T-DP-MONITORING — `92269e7` (#109) (**Stage 11 deployment ✅ 6 / 6 — CLOSED**; inert-until-secrets observability scaffolding mirroring the other Stage 11 deploy paths (each live path guarded on its provider secret, skipped not failed). Sentry (errors) + PostHog (analytics) init seams with **injectable hooks** so no provider SDK is a dependency yet (dependency-light per brief): web `apps/web/lib/observability/` (`initWebObservability` mounted via `<ObservabilityInit/>` in the root layout), mobile `apps/mobile/src/lib/observability/` (`initMobileObservability` called from `app/_layout.tsx`), python `apps/api-python/observability/` (`init_sentry`, lazy `sentry-sdk` import, no-op when absent — effective once the Q-021 HTTP entrypoint lands). Uptime/health: web `GET /api/health` liveness 200 (deep curl+DB+R2 cron+pager deferred → #FU-63); python `/healthz` blocked on Q-021. CI `.github/workflows/deploy-monitoring.yml` creates a Sentry release + associates commits, guarded on `SENTRY_AUTH_TOKEN` (skipped, never failed, when absent — same guard as deploy-web/fly/db/mobile). Docs: `infra/monitoring/README.md` runbook + `infra/DEPLOYMENT_SECRETS.md` §6 + per-app `.env.example` monitoring blocks. No DB migrations; local CI green (lint + typecheck + build + **1638 mobile vitest** + web observability/health tests + **6 pytest** observability). Raised **Q-025** (production deep health-check + pager scaffolded, not operational; #FU-63). Go-live still gated on Pablo's secrets (#FU-53) + the Q-021 Python entrypoint) — **iter 35**
- T-SH-THEMES — `agent/T-SH-THEMES` (**Stage 08 shareables ✅ 3 / 3 — CLOSED**; pro-gated themed public shareable pages. Typed registry of 5 flat themes (`default` free; `dark`/`paper`/`neon`/`gold` Pro) in `apps/web/app/c/themes/`, pinned to the `SHAREABLE_THEMES` enum; `resolveTheme` (stale/unknown/null → default) + `resolvePublicTheme(id, ownerIsPro)` (free-tier downgrade). SSR-safe `<ShareableThemeProvider>` paints palette/font/header-band + exposes `--share-*` vars + `useShareableTheme()`; `<ShareableView>` wraps content + colours header/empty-state from the palette. Settings `<ThemePicker>` gallery (live `<ThemeThumbnail>` swatches) replaces the placeholder `<select>`, **pro-gated via the merged `useGate('shareable_themes')` hook (T-PB-GATING) — not the entitlements fallback**; free users preview but a Pro pick is not persisted + surfaces `<UpgradePrompt>` → `/billing`. `ogPaletteForTheme` exports a flat Satori-friendly palette for a later OG-theming wire-up (no heavy `lib/og/` edits). **+62 web vitest (861 web total)**; lint / typecheck / build all green locally (Node 22.13.0; Actions skipped per quota policy). **Q-024 raised** (public share payload lacks owner tier → server-side free-downgrade render can't enforce; interim renders the stored theme for everyone, downgrade branch pre-wired) → **#FU-61** (T-BE-SHAREABLE-OWNER-TIER); OG-theming wire-up → **#FU-62** (T-SH-OG-THEME-WIRE). Recovered from a concurrent duplicate-writer that twice reset the worktree + injected a divergent `ThemeSwatch` design — imposed the canonical `ThemeThumbnail`/`useGate` design + pushed to origin early for durability) — **iter 35**
...
1. **`T-DL-DB-TEST-INFRA` (proposed)** — vitest in `@binderly/db`;
   ...
   IPC-pipe failures. ✅ **CLOSED (backlog burndown; 2026-05-29)** —
   `db:generate`/`db:migrate` are now plain-ESM `scripts/*.mjs` (no
   `tsx`); pure helpers extracted + covered by vitest (`__test__/`,
   24 tests: 13 migrate, 11 generate). `migrate.mjs` verified live
   against Supabase :54322 (applied 25 migrations).
2. **`T-DL-PRICING-TYPES-CONSOLIDATION` (proposed)** — hoist a shared
   ...
   ✅ **CLOSED (backlog burndown; 2026-05-29)** — single
   `RawPriceObservation` (+ `rawPriceObservationSchema`) now lives in
   `data-pipeline/src/types.ts`; the aggregator re-exports it (same
   symbol, no barrel collision) and EBAY-BROWSE imports it directly.
   `RawEbayBrowsePriceObservation` retired. `parseConfidence`
   standardised as a `numeric(3,2)` string to match the DB column.
3. **`T-DL-DATAPIPELINE-DOTENV` (proposed)** — auto-load `.env` in
   ...
   migrations live); fix the README. ✅ **CLOSED (backlog burndown;
   2026-05-29)** — new `data-pipeline/src/load-env.ts` auto-loads
   `.env` (package-local then monorepo root, never overwriting set
   vars) and is imported first in all 5 `scripts/*.ts`; pure parser
   covered by `load-env.test.ts` (7 tests). README rewritten to point
   every smoke test at Supabase :54322 (`supabase start`), not the
   Compose :5433 stub.
4. **`T-DL-DB-TURBO-BUILD-PIPELINE` (proposed; Q-004 follow-up)** — add
   a turbo `^build` pipeline so `pnpm seed` implicitly builds
   `@binderly/db` first. ✅ **CLOSED (backlog burndown; 2026-05-29)** —
   `^build` was already on `build`/`test`/`typecheck` (already-done),
   but `pnpm seed` bypassed turbo entirely. Added a `seed` task to
   `turbo.json` (`dependsOn: ["^build"]`, `cache: false`) + a root
   `"seed": "turbo run seed --"` script so db consumers build
   `@binderly/db` first.
5. ✅ **CLOSED** (`664c1e0`, #114 — migration `0028_pricing_current_view_v2.sql` recreates `mv_current_price` **additively**: every v1 column preserved + appended `current_price` (30-day median), `trend_30d_pct` / `trend_90d_pct` / `trend_all_time_pct`, derived `trend_direction`, `sample_count_30d`, and `last_observation_at` (`MAX(observed_at)` from the raw log). Trends are calendar-anchored on the slice's own latest `period_start` (deterministic between refreshes). Grants tightened to SELECT-only to realize the v1 migration's documented intent (Supabase default-privileges quirk — same root cause as #FU-28 / `0020`). Contracts: optional trend fields + `priceTrendDirectionSchema` on `currentPriceDto` + `printingCurrentPriceDto`; the `GET /v1/printings/:id/current-price` Edge handler now surfaces them. api-contracts +13 / edge-fn currentPrice +7 tests; fresh `db:migrate` apply + `verify-rls` 137/0 green; trend math validated on a synthetic series. Display semantics + freshness-source decision → **Q-028**; consumer-side UI wiring → **#FU-66**). **`T-DL-PRICING-CURRENT-VIEW-V2` (proposed; Q-006 follow-up)** — pair with T-SP-PRICING-DISPLAY (Phase 2). Adds the richer trends + freshness shape from `context/data-model.md` § `mv_current_price`.
...
7. **`db:migrate` UX wart** — `migrate.ts` errors hard if
   ...
   gracefully. ✅ **CLOSED (backlog burndown; 2026-05-29)** —
   `migrate.mjs` now degrades gracefully: a missing or empty journal
   logs a clear "nothing to apply" notice and exits 0; a journal that
   references SQL files absent on disk still errors loudly (real
   corruption). Covered by `__test__/migrate.test.ts`.
8. **Dependabot backlog** — ~9 open PRs from when CI landed.
9. **`.nvmrc` 22.22.2 not locally installable** — fall back to 22.13.0.
   ✅ **CLOSED (backlog burndown; 2026-05-29)** — `.nvmrc` and root
   `package.json` `engines.node` both pinned to `22.13.0` (verified
   installable + used for the full local CI battery).
10. **`scripts/cleanup_worktree.sh` regex rejects single-letter task scopes** — script enforces `^T-[A-Z]{2}-[A-Z0-9-]+$` but iter 14's `T-M-SHELL` and `T-W-SHELL` only have one letter in the scope segment. Orchestrator did manual `git worktree remove --force` + `git branch -D` for both. Either relax the regex to `^T-[A-Z]+-[A-Z0-9-]+$` or rename the affected tasks. Same regex appears in the repo's `pr-title` lint workflow (which is why both shell PRs needed `feat(<area>): T-X-SHELL — …` reformatting at merge time). ✅ **CLOSED (backlog burndown; 2026-05-29)** — relaxed to `^T-[A-Z]+-[A-Z0-9-]+$` in all four locations: `scripts/{cleanup_worktree,spawn_worktree,list_ready_tasks}.sh`, `scripts/README.md`, and the `.github/workflows/ci-pr-title.yml` regex line. Single-letter scopes like `T-M-SHELL` / `T-W-SHELL` now pass.
11. **`@binderly/web` worker reported all-green-locally but CI build failed.** Root cause: AuthProvider eagerly called Supabase client constructor (which throws on missing env) inside useMemo at render time; `next build` static prerender evaluates this for every page. Orchestrator hotfix `fb4a6d0` deferred init to useEffect. **Action item:** add a CI-style "build with no env" smoke test the worker can run locally before pushing, so future Next.js shells catch this in the dispatch loop instead of post-merge. ✅ **CLOSED — T-POLISH-SWEEP (`fc18834`, #110, iter 35).** Added `apps/web` `build:no-env` script (strips every `NEXT_PUBLIC_*`/`SUPABASE_*` var then asserts `next build` exits 0 — the lazy-init Supabase pattern must survive an env-less build) + wired it as an explicit step in the CI `build` job + a Node-script eslint override for `apps/web/scripts`. Runnable locally before pushing.
...
14. **Dead M-SHELL placeholder screens** at `apps/mobile/src/screens/SignInScreen.tsx` and `AuthCallbackScreen.tsx` (legacy duplicates from before T-M-AUTH repointed the route shells). Outside any current task's owns_paths; flag for an M-SHELL cleanup follow-up. ✅ **CLOSED — T-POLISH-SWEEP (`fc18834`, #110, iter 35).** Deleted `apps/mobile/src/screens/SignInScreen.tsx` + `AuthCallbackScreen.tsx`; only the screens smoke matrix referenced them (updated). The live auth screens live in `src/screens/auth/`.
15. **`apps/web/components/providers/AuthProvider.tsx` not prettier-compliant** — `pnpm --filter @binderly/web format:write` reformats it. T-W-SHELL committed it in this state and `format:check` isn't a CI gate, so workers can't safely re-run format on the file. Worth a one-shot cleanup commit. ✅ **CLOSED — T-POLISH-SWEEP (`fc18834`, #110, iter 35).** `prettier --write` on the file (formatting only, no behaviour change).
...
18. **URL convention divergence between web and mobile browse routes.** T-W-BROWSE uses raw UUIDs for `/sets/[id]` and `/cards/[id]` (api-client only exposes by-id). T-M-BROWSE uses slug-based `/sets/[slug]` resolving against the `/v1/sets` list cache (e.g. `en-base1`). Both work; both shipped green. T-W-COLLECTION inherited UUID; T-M-COLLECTION inherited slug — divergence persists in iter 18. Future cross-platform consolidation: pick one convention (probably slug, after a `getSetBySlug` endpoint lands) and migrate the other. Low priority — neither is user-visible while routes are SSR-hidden. **Logged as #FU-18.** ⚠️ **PARTIALLY CLOSED — T-POLISH-SWEEP (`fc18834`, #110, iter 35).** Investigation split this into two independent divergences. **`/sets/*`** (UUID `set.id` vs slug `set.canonicalKey`) is mechanically alignable with no new endpoint/migration — **chosen convention = slug**. **`/cards/*`** is NOT a slug/id swap: web `/cards/[id]` is a *printing* UUID → `getPrinting()`, mobile is a *card* UUID → `getCard()` (same path, different entity) — unifying needs a product decision (printing- vs card-centric) + likely a by-slug endpoint. No safe polish-sized refactor; documented the choice + raised **Q-026**; full cross-platform alignment logged as remainder **#FU-64**.
...
20. **`@binderly/ui` `<Modal>` primitive missing.** T-W-CUSTOM rolled a modal in-tree at `apps/web/components/collections/custom/Modal.tsx` (Esc + backdrop close, ARIA dialog, no focus trap). T-W-SMART likely makes the same trade-off; iter-20+ tasks will too. Proposed `T-SP-UI-MODAL` follow-up to hoist a shared cross-platform modal into `@binderly/ui` once 2+ consumers are in main. **Logged as #FU-20.** ✅ **CLOSED — T-POLISH-SWEEP (`fc18834`, #110, iter 35).** Added cross-platform `<Modal>` to `@binderly/ui` (backdrop + centred surface, Esc/backdrop close, dialog a11y; built on `@tamagui/core` like the other primitives); migrated the 3 web custom-collection consumers (create/delete/add-cards) onto it + deleted the in-tree `apps/web/.../custom/Modal.tsx`. +6 vitest.
21. **`@binderly/ui` `<Input>` doesn't expose `onBlur` / `onEndEditing`.** T-W-CUSTOM's detail screen uses raw `<input>`/`<textarea>` for inline-edit name/description because the shared `<Input>` wrapper doesn't surface blur events. Future T-SP-UI-INPUT-BLUR adds the prop pass-through (additive). **Logged as #FU-21.** ✅ **CLOSED — T-POLISH-SWEEP (`fc18834`, #110, iter 35).** `<Input>` now exposes `onBlur` + `onEndEditing`, forwarded through the underlying primitive's blur for a consistent cross-platform "finished editing" signal. +4 vitest.
...
28. ✅ **MOOT — already shipped.** Migration `0020_revoke_admin_debug_view_grants.sql` (merged on `main`, explicitly "closes #FU-28") already ships the `REVOKE ALL ON public.v_pg_stat_statements_top_queries FROM anon, authenticated;`. Re-verified during T-DL-DB-VIEWS (#114, `664c1e0`): a fresh `pnpm --filter @binderly/db verify-rls` reports **137 passed / 0 failed** — both `anon` and `authenticated` are blocked from the view, `service_role` allowed. No new migration needed; the prior 2-failure baseline is resolved. **One-line REVOKE migration for `v_pg_stat_statements_top_queries` (`T-DL-RLS-PG-STAT-REVOKE` proposed).** `pnpm --filter @binderly/db verify-rls` reports 2 pre-existing failures on this view (anon + authenticated can read it). Cause: Supabase's default `ALTER DEFAULT PRIVILEGES … GRANT ALL ON TABLES TO anon, authenticated` fires on view creation **before** migration `0016_admin_debug_views.sql`'s `REVOKE ALL FROM PUBLIC` runs (`PUBLIC` ≠ the union of all roles). Fix: ship `0020_*.sql` with `REVOKE ALL ON public.v_pg_stat_statements_top_queries FROM anon, authenticated;`. Surfaced by the T-BE-Q013-CLEANUP worker (PR #73). **Not introduced by #73** — predates branch base on main commit `b13d3ed` (T-DL-ADMIN-DEBUG-SURFACES, iter 11). Trivial to land. **Logged as #FU-28.**
...
31. **#FU-31 — T-GR-CAPTURE-FULL-SCHEMA (T-GR-CAPTURE-UX follow-up).** The remaining 2 corner crops (bottom-left, bottom-right) + the surface raking-light shot from PROJECT.md § 12 weren't shipped in v1. T-GR-CENTERING + T-GR-CORNERS only need the 4 already shipped (front + back full, front-corner top-left, back-corner top-right). Full schema adds 3 capture steps + 3 quality gates + 3 overlay framings to the existing 4-shot session. **Logged as #FU-31.** ✅ **CLOSED** (`84d1123`, #115 — extended the guided capture flow to the full PROJECT.md § 12 shot set: bottom-left + bottom-right corner crops + the surface raking-light shot; 4 → 7 capture steps + per-step quality gates + overlay framings; four corners map to `cornerUrls` `[TL, TR, BL, BR]`, surface → `surfaceUrl`; append-only api-contracts capture model (`gradingCaptureShotSetSchema`, no DB migration). Mobile 1668 / api-contracts 306 green.)
32. **#FU-32 — T-GR-CENTERING-ROUTING (T-GR-CAPTURE-UX follow-up).** Session hand-off from capture → centering uses a module-scoped ref `__getLastEmittedSession()` tagged `@deprecated`. T-GR-CENTERING swaps in a real router-param strategy — likely expo-router params + a stable session id, since the 4 captured stills are too large to serialise into URL params and need to stay in-memory. **Logged as #FU-32.** ✅ **CLOSED** (`ef3bda6`, #118 — replaced the `@deprecated` module-scoped ref with the in-memory session-store + a real expo-router `sessionId` param: capture `storeSession()`s the emitted session then pushes `/grading/centering?sessionId=<id>`, and the review/centering screens read it back via `useLocalSearchParams`; the `__getLastEmittedSession`/`__setLastEmittedSession` exports are gone.)
33. **#FU-33 — live frame-processor sample for capture-button quality state (T-GR-CAPTURE-UX follow-up).** Today `attemptCapture` is screen-injected and runs only on capture-button tap. The `CaptureAttemptInput` shape is already designed to compose a live frame-processor quality stream with `takePhoto()` once a CPU-side JPEG decoder is bundled. UX win: capture button can disable / show "hold steady" / hint corner-framing hints before the user even taps. **Logged as #FU-33.** ✅ **CLOSED** (`84d1123`, #115 — live vision-camera frame-processor sample makes the capture button reflect real-time quality (sharpness/brightness/coverage) before tap, reusing `evaluateCaptureQuality` via `useCaptureFrameProcessor` + a `CaptureQualitySink` + the pure `deriveCaptureButtonState`; behind the existing frame-processor abstraction, gracefully no-ops without a native camera / `toArrayBuffer` (tests run cameraless), ~10 FPS throttle. Raised Q-029 — the HWC-RGB pixel-format assumption needs on-device validation, tracked as #FU-67.)
34. **#FU-34 — scanner thumbnail URL plumbing (T-SC-UX follow-up).** `MatchOverlay` + `DisambigPicker` show coloured placeholders today; need `MatchResult` to expose a thumbnail URL or a `usePrinting()` lookup hook reading from the api-client. Small backend addition (the printing already has an image URL in the catalogue; just needs to flow through the match result DTO) + 2 UI component swaps. **Logged as #FU-34.**
35. **#FU-35 — scanner model warmup cache (T-SC-UX follow-up).** `useModelLoader` re-loads the TFLite model + ANN index on every `ScanScreen` remount; tab navigation costs ~200-400 ms of warmup. A module-level singleton cache keyed on `model + index` paths would keep both warm across tab switches. Decision needed on lifetime/eviction (likely indefinite for v1 since both assets are small and shipped in-bundle). **Logged as #FU-35.**
...
40. **#FU-40 — `T-GR-DATA-PRINTING-MATCH` (proposed; iter 28 cross-cutting follow-up).** Resolve the NULL `printing_id` FK on grading observation tables (`grading_training_sample` rows from PSA + eBay, `auction_lot_observation` rows from PWCC + Goldin). All 3 scrapers leave `printing_id` NULL intentionally — fuzzy matching against the canonical `printings` table is its own task. Likely strategy: trigram + ANN search over `(card_name, set_name, card_number)` tuples + a confidence threshold + a manual-resolution surface in admin UI for low-confidence matches. Substantively a backend Python task with its own pytest battery. **Logged as #FU-40.** ✅ **CLOSED — T-GR-DATA-HYGIENE (`26d334a`, #107, iter 35).** Shipped pure-Python `apps/api-python/grading/matching/` (no new deps, runs in CI — no trigram/ANN): observation-title parser + `difflib` composite-score `PrintingMatcher` (`EXACT`/`FUZZY`/`AMBIGUOUS`/`NO_MATCH`; ambiguous + no-match deliberately leave `printing_id` NULL) + `binderly-backfill-printing-match` **dry-run-by-default** job (live UPDATE via Supabase PostgREST behind an env gate) over all 3 observation tables + JSON fixtures + unit tests (exact/fuzzy/ambiguous/no-match). No DB migration. The admin manual-resolution surface for low-confidence matches is left unbuilt (out of scope; the `AMBIGUOUS` bucket is the seam).
...
54. ✅ **CLOSED** (`8a4bb5e`, #106 — structured fallback signal at the `@binderly/entitlements` fallback site + injectable hook). **#FU-54 — Entitlement-fallback observability (proposed; T-PB-ENTITLEMENTS follow-up; iter 33; renumbered from 53 — the parallel T-DP-INFRA worker claimed #FU-53).** The `@binderly/entitlements` RC read client + the `GET /v1/me/entitlements` Edge handler are deliberately **fail-closed**: any RC outage / non-200 / malformed body / missing `REVENUECAT_SECRET_API_KEY` silently degrades every caller to `{ tier: 'free', source: 'fallback' }`. That's the correct UX (never 500, never over-grant), but it also means a *prolonged* RC outage or a key misconfiguration would silently downgrade the whole user base to free with no alarm — today the only signal is a `console.warn`. Follow-up: emit a structured metric/log on every `source: 'fallback'` (count + reason), wire a threshold alert (e.g. fallback rate > N% over 5 min → page on-call), and consider a short-TTL server-side cache of the last-known-good RC read per user so a transient blip doesn't flap paying users to free between requests. Small additive observability task; no API change (the DTO already carries `source`). **Logged as #FU-54.**
55. ✅ **CLOSED** (`8a4bb5e`, #106 — pro-gated `POST /me/community-submissions` Edge handler shipped). **#FU-55 — `T-BE-COMMUNITY-SUBMISSION-ENDPOINT` (proposed; T-GR-COMMUNITY-FLYWHEEL follow-up; iter 34).** Ship the `POST /v1/me/community-submissions` Edge handler that backs the already-merged `client.communitySubmissions.submitCommunitySubmission()` resource + `submitCommunitySubmissionRequest`/`submitCommunitySubmissionResponse` contracts. The handler: (a) re-validates the body server-side (the client validators in `apps/mobile/src/grading/community/validation.ts` mirror, but never replace, server validation), (b) **pro-gates** via the same RC entitlement read as `GET /v1/me/entitlements` (`grading_prediction`) → 403 for free users, (c) normalizes the cert number + upserts into `community_submission` keyed on `(user_id, grade_company, cert_number_normalized)` returning `alreadySubmitted: true` on an idempotent re-submit, and (d) maps to the `communitySubmissionDto` response shape. This intentionally lands after the client + contract (same pattern as the `entitlements`/`grading` resources preceding their handlers); until it deploys the mobile flow surfaces the network error state. **Logged as #FU-55.**
56. **#FU-56 — Cross-company grade calibration + flywheel job runner wiring (proposed; T-GR-COMMUNITY-FLYWHEEL follow-up; iter 34).** The flywheel ingestion (`apps/api-python/grading/flywheel/`) currently normalizes each company's grade onto the **same** 1–10 0.5-step grid and stores the raw company + grade alongside — a conservative mechanical mapping, NOT a calibrated cross-company equivalence (a BGS 9.5 ≠ a PSA 9.5 in practice). Proper calibration is a real ML/product decision: fit a per-company → canonical-PSA-scale transform (mirroring `rules/07-grading.md`'s PSA-calibration posture for the aggregator) once enough paired/overlap data exists. Separately, wire `run_ingestion`'s mock-by-default `db_upsert_fn` / `existing_source_id_fn` seams to a live DB job runner (reading opted-in `community_submission` rows → writing `grading_training_sample`) behind a live env gate, mirroring the iter-28 scraper job wiring. Image transcode/storage for submitted photos rides #FU-39 (today image refs are persisted as URLs). **Logged as #FU-56.** ✅ **CLOSED (calibration scaffold) — T-GR-DATA-HYGIENE (`26d334a`, #107, iter 35).** Shipped `apps/api-python/grading/calibration/`: PSA-anchored per-company affine placeholder map (numpy dot, low-confidence heuristic constants, clamped `[1,10]`) + `GradeCalibrator`/`normalize_grade` + tests + README. **Scaffold only** — learned cross-company calibration is spun out to **#FU-60** (see Q-023); the live flywheel `db_upsert_fn`/`existing_source_id_fn` job-runner wiring portion of this FU remains a separate follow-up (untouched here).
57. ✅ **CLOSED** (free single-card-scan mode shipped + continuous/stack scanner gated behind Pro via `useGate('stack_scanner')`; new `<ScanModeToggle>` (Single | Stack·Pro) + `<SingleCardConfirm>`; free → single only, Pro → both modes; mode-toggle-vs-rules/06 "no user toggle" tension logged as Q-030 — squash `71918fa`, #116). **#FU-57 — Gate the continuous (stack) scanner behind Pro + build the free single-card-scan mode (T-PB-GATING follow-up; iter 34).** PROJECT.md § 16 specifies "Stack scanner (continuous) — pro only (free = single-card scan)", but `apps/mobile/src/screens/scan/ScanScreen.tsx` only ships the continuous-scan UX (auto-add loop + auto-detected stack-review phase) — there is no single-card-scan mode to fall back to for free users, so there is no clean seam to attach a gate to without reworking the scanner (out of T-PB-GATING's add-a-gate-check boundary). Follow-up: (a) build a single-card-scan mode (one capture → one match → confirm, no continuous accumulation) as the free experience; (b) gate the continuous/stack mode with `useGate('stack_scanner')` from `@binderly/feature-flags` (the hook + `stack_scanner` `PaidFeature` are already shipped) — when free, the "Done"/stack-review path (or a "continuous mode" toggle) routes to `<UpgradePrompt>` instead. The decision core + mobile glue are ready; this is a scanner-feature task, not a gating task. **Logged as #FU-57.**
58. **#FU-58 — Wire EAS Update (OTA) + decide mobile auto-build cadence (proposed; T-DP-EAS / Q-022 follow-up; iter 35).** The EAS scaffolding ships build/submit only: `apps/mobile/eas.json` declares `development`/`preview`/`production` **channels** so [EAS Update](https://docs.expo.dev/eas-update/introduction/) can be layered on, but the app has no `expo-updates` dependency or runtime config, so JS-only hotfixes still need a full store build. Follow-up: add `expo-updates` + a runtime-version policy + update-check UX (mobile-track app code, not deploy config), and decide whether to auto-build a `preview` profile on merge to `main` vs the current manual-`workflow_dispatch`-only cadence (native builds are metered). Also note `extra.eas.projectId` is written by Pablo's `eas init` (first-run, against his Expo account) and so isn't committed by the scaffolding. Gated on the Expo account/token (Stage 11 go-live, #FU-53). **Logged as #FU-58.**

59. ✅ **CLOSED** (page-walk swapped for the #FU-49 single-GETs; `MAX_PAGE_WALK` cap + walk-limit `transient_error` branch dropped — `43664d5`, #113). **#FU-59 — Swap the T-OF-CONFLICTS `ServerFetcher` page-walk for the new single-GETs (proposed; T-BE-READS-WRITES / #FU-49 follow-up; iter 35).** #FU-49 shipped the api-client single-GETs (`collection.getCollectionItem` / `getCustomCollectionItem`) + the matching Edge routes, but the original consumer — `apps/mobile/src/sync/conflicts/`'s `ServerFetcher` adapter, which still walks pages client-side and gives up at `MAX_PAGE_WALK = 50` (~5 000 items) → `transient_error` — was left untouched (offline-sync track, out of this backend cluster's scope). Follow-up: swap the page-walk for the direct single-GETs behind the same `ServerFetcher` interface (no resolver changes; the adapter is the only consumer) and drop the `MAX_PAGE_WALK` cap + the `transient_error` walk-limit branch. Small mobile-track change (~30 LOC + tests). **Logged as #FU-59.**
60. **#FU-60 — Learned cross-company grade calibration (proposed; T-GR-DATA-HYGIENE / #FU-56 follow-up; iter 35).** #FU-56's calibration half shipped as a **scaffold**: `apps/api-python/grading/calibration/` ships a PSA-anchored per-company **affine placeholder** map (numpy dot of heuristic, low-confidence `slope`/`intercept` constants, clamped `[1,10]`) behind `GradeCalibrator`/`normalize_grade`, `CALIBRATION_VERSION='v0-placeholder'`. Replace the constants with a **data fit** once the flywheel accumulates labelled cross-company pairs (same physical card graded by ≥2 companies, or strong card-identity matches via #FU-40's printing match + realised market value as the regression target): fit per-company (or per-company-per-grade-tier) `slope`/`intercept`, inject via `GradeCalibrator(calibrations=...)` (no call-site change — the affine is already a numpy dot so a learned weight vector drops in), and bump `CALIBRATION_VERSION`. Open design questions (signal source, single-affine vs per-tier, PSA-anchored vs company-neutral latent scale) tracked as **Q-023**. **Logged as #FU-60.**
 61. ✅ **CLOSED** (owner tier on the public payload + server-side free-theme downgrade on the public render path; Q-024 resolved — `43664d5`, #113). **#FU-61 — Surface the shareable owner's tier on the public payload for server-side free-theme downgrade (proposed; T-SH-THEMES / Q-024 follow-up; iter 35) — `T-BE-SHAREABLE-OWNER-TIER`.** T-SH-THEMES' `resolvePublicTheme(themeId, ownerIsPro)` already force-renders the `default` theme when `ownerIsPro === false`, but the merged read path (`publicShareableDto` in `@binderly/api-contracts`, `PublicSharePayload` in `apps/web/lib/share/api.ts`, and the anonymous `publicShareable` Edge handler) exposes only `{ handle, displayName, avatarUrl, bio }` for the owner — no tier — so the public page currently renders the stored theme for everyone (interim). Follow-up: add `owner.tier` (or `ownerIsPro: boolean`) to the contract + Edge handler (entitlements read for the resolved owner), then flip `ShareableView`'s `resolvePublicTheme(theme, null)` → `resolvePublicTheme(theme, ownerIsPro)` (the downgrade branch is already wired + tested, so no theme-system change is needed). Backend edge-fn + contract change, outside T-SH-THEMES' owns_paths. See **Q-024**. **Logged as #FU-61.**
 62. **#FU-62 — Wire theme palettes into the OG image hero (proposed; T-SH-THEMES follow-up; iter 35) — `T-SH-OG-THEME-WIRE`.** T-SH-THEMES exports a flat, Satori-friendly palette per theme via `ogPaletteForTheme(themeId)` (`apps/web/app/c/themes/og-palette.ts`) but stops short of editing the OG renderer (T-SH-OG-IMAGES owns `apps/web/lib/og/` + `apps/web/app/api/og/`, and the brief capped this task at a read-only palette export to avoid heavy `lib/og/` edits). Follow-up: in the OG route, resolve the shareable's `theme_id` → `ogPaletteForTheme` and feed the hero background / wordmark / title / tile / footer colours from it (respecting the same free-tier downgrade as the public page once #FU-61 lands), replacing the current fixed brand gradient. Small additive change in the OG worker's scope. **Logged as #FU-62.** ✅ **CLOSED** (`c2ddd33`, #117 — the OG hero (background gradient / wordmark / title / tiles / footer) now reads from `ogPaletteForTheme(shareable.theme)` resolved through `resolvePublicTheme` so the SAME free-tier downgrade as the public page applies (a free owner's stored Pro theme reverts to the default palette); a palette-derived 135° gradient replaces the fixed brand gradient. Render + route tests cover the Pro-theme palette + the free-owner downgrade.)
...
40. **#FU-64 — Cross-platform `/sets/*` + `/cards/*` URL convention alignment (T-POLISH-SWEEP / #FU-18 remainder; iter 35).** #FU-18's investigation split the divergence in two. **`/sets/*`** is mechanically alignable without a new endpoint or migration — both `SetDto.id` (UUID) and `SetDto.canonicalKey` (slug, e.g. `en-base1`) exist and mobile already resolves slug→id by scanning `/v1/sets`; **chosen convention = slug (`canonicalKey`)** (friendlier URLs, matches mobile, status.md follow-up #18's stated preference). **`/cards/*`** is NOT a slug/id swap: web `/cards/[id]` is a **printing** UUID → `cards.getPrinting()` (set+card+variant in one fetch), mobile `/cards/[id]` is a **card** UUID → `cards.getCard()` — same path, different entity. Unifying needs a product decision (printing- vs card-centric detail) + likely a `getSetBySlug`/`getCardBySlug` endpoint (no migration — `canonical_key` columns already exist + are unique). Deferred from the polish sweep as larger/riskier than polish. **Logged as #FU-64; see Q-026.** ⚠️ **PARTIALLY CLOSED — `/sets/*` DONE** (`ef3bda6`, #118 — `/sets/*` aligned to the `canonicalKey` slug across web + mobile via a shared `cards.getSetBySlug`; web route renamed `[id]`→`[slug]` with legacy-UUID→slug `router.replace` redirects). **`/cards/*` REMAINS OPEN** (printing-vs-card entity product decision still blocked on **Q-026**). Slug-resolution optimization follow-up logged as Q-031/#FU-69.
41. **#FU-66 — Wire `mv_current_price` v2 trends + `lastObservationAt` into the card-detail UI (proposed; T-DL-PRICING-CURRENT-VIEW-V2 / #FU-5 follow-up; iter 36) — `T-SP-PRICING-DISPLAY-TRENDS`.** Migration `0028` + the `GET /v1/printings/:id/current-price` handler now expose `currentPrice` (30-day median), `trend30dPct` / `trend90dPct` / `trendAllTimePct`, `trendDirection`, `sampleCount30d`, and `lastObservationAt` (all optional on `printingCurrentPriceDto`), but no consumer renders them yet. Follow-up: surface the headline trend arrow + % change + "last seen" on card detail (web `CardPriceBlock` + mobile `PriceBlock`), and decide whether the displayed `freshness` should switch from `computedAt` (rollup recency) to the new `lastObservationAt` (observation recency) — see **Q-028** for the display-semantics, trend-base, ±1% dead-band, and freshness-source questions. Pure consumer-side change (the data + contract + endpoint already shipped). **Logged as #FU-66.** ✅ **CLOSED** (`c2ddd33`, #117 — web `CardPriceBlock` + mobile `PriceBlock` surface the headline 30-day trend arrow + signed % change + a "Last seen" line; new shared pure formatters added to `@binderly/pricing-display` (`formatTrendPercent`/`trendArrow`/`trendDirectionLabel`/`hasRenderableTrend`). **Q-028 RESOLVED**: trend base + ±1% dead-band kept server-side (UI renders `trendDirection`/`trend30dPct` verbatim, no client recompute); `freshness` stays anchored on `computedAt` (no bucket re-tuning) while `lastObservationAt` is surfaced additively as the truer observation-recency line — flipping the band itself is deferred to #FU-68.)
42. **#FU-67 — On-device validation of the live capture frame-processor pixel format (proposed; T-GR-CAPTURE-FULL / #FU-33 follow-up; iter 37) — `T-GR-CAPTURE-LIVE-DEVICE-VALIDATION`.** #FU-33's live frame-processor feeds `frame.toArrayBuffer()` straight into the quality gate assuming an HWC-RGB `Uint8Array` (mirroring the scanner detect worklet). vision-camera v4 can hand the worklet YUV / BGRA / RGB depending on platform + `pixelFormat`; the worklet gracefully no-ops on a `w·h·3` length mismatch, so a wrong format degrades to tap-driven capture rather than bogus live feedback. Follow-up: pin `pixelFormat="rgb"` on `<Camera>` for the capture screen + validate on Pablo's device matrix (add a CPU-side YUV/BGRA→RGB decode only if a target device won't deliver RGB). See **Q-029**. **Logged as #FU-67.**
43. **#FU-68 — Switch the headline `freshness` band from `computedAt` (rollup recency) to `lastObservationAt` (observation recency) (proposed; T-SP-PRICING-DISPLAY-TRENDS / Q-028 follow-up; iter 37) — `T-SP-FRESHNESS-OBSERVATION-RECENCY`.** #FU-66 surfaced `lastObservationAt` as an additive "Last seen" line but deliberately left the `fresh`/`stale`/`stale_old` band derived from `computedAt` for backward-compatibility (Q-028). Follow-up: re-derive the band from `lastObservationAt` and re-tune the 7d/30d bucket thresholds against the real observation cadence (today's thresholds were sized for rollup recency, not observation recency). Pure server-side derivation change in the `GET /v1/printings/:id/current-price` handler + the freshness bucket constants. **Logged as #FU-68.**
