...
## Q-44 — `ml_common` should be generalised for multi-subgrade support

**Raised by:** T-GR-EDGES sub-agent (iter 30, 2026-05-20).
**Blocking:** No. T-GR-EDGES works around it.

**Context:**

`ml_common.MergedDataLoader` and `ml_common.types.LabelledGradingSample` are
hard-coded to the `corners` sub-grade column:

- `LabelledGradingSample` has a `corners_score` field (not a generic `score`).
- `MergedDataLoader.load_labelled()` calls `is_labelled_for_corners()` —
  ignoring the `subgrade_column` constructor parameter that exists but is
  unused.
- The individual loaders (`PSADataLoader`, `EbayDataLoader`,
  `AuctionDataLoader`) hard-code `subgrades.get("corners")`.

T-GR-EDGES introduced a parallel `EdgesMergedDataLoader` +
`EdgesLabelledSample` in `apps/api-python/grading/edges/dataset.py` rather
than modifying `ml_common` while T-GR-SURFACE was running in parallel.
T-GR-SURFACE will likely introduce the same parallel boilerplate.

**Options:**

1. **Generalise `ml_common` now** (post-T-GR-SURFACE): add a
   `subgrade_column: str` parameter to `LabelledGradingSample` (rename
   `corners_score` → `score`), make all loaders and filters
   column-agnostic, and collapse the per-sub-grade data layers into a
   single reusable `MergedDataLoader`.
2. **Leave per-sub-grade loaders as-is**: the boilerplate is small (≤50
   lines per sub-grade), and the explicit types (`EdgesLabelledSample`,
   `SurfaceLabelledSample`) are clear in intent.

**Recommendation:** Option 1 after T-GR-SURFACE merges — a single
refactoring task `T-GR-ML-COMMON-GENERALISE` that touches
`ml_common/`, `corners/`, `edges/`, and `surface/` in one PR.

**Status: open; not blocking.** Resolves when T-GR-SURFACE lands or when
orchestrator schedules the cleanup task.
