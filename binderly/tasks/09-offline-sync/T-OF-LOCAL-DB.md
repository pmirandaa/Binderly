...
> **Steps to elaborate:**
>
> 1. Read `PROJECT.md` (especially § 15 (Offline Mode)) and any
>    referenced sections.
> 2. Read `rules/09-offline-sync.md` (the stage rules).
...
> 8. Commit as `docs(tasks): elaborate T-OF-LOCAL-DB`.
...
Mobile SQLite schema mirror + local repository layer.
...
- PR title: `T-OF-LOCAL-DB: Mobile SQLite schema mirror + local repository layer`
