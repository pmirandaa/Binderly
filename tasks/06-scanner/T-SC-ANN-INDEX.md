...
> 8. Commit as `docs(tasks): elaborate T-SC-ANN-INDEX`.
...
- `apps/api-python/ann/`
- `apps/mobile/src/scanner/ann/`

## Provisional goal

ANN index build (server) + on-device load.
...
- PR title: `T-SC-ANN-INDEX: ANN index build (server) + on-device load`
