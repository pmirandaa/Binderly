...
describe('<ShareablesSettingsScreen> (mobile)', () => {
  it('renders the page shell after loading the profile + shareables', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('m-settings-shareables-page')).toBeInTheDocument();
    });
    expect(screen.getByTestId('m-profile-fields-card')).toBeInTheDocument();
    expect(screen.getByTestId('m-settings-shareables-list')).toBeInTheDocument();
    expect(screen.getByTestId('m-settings-shareables-create-card')).toBeInTheDocument();
  });
  ...
  it('renders the empty state when the user has no shareables', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-settings-shareables-empty')).toBeInTheDocument();
    });
  });
  ...
  it('shows the free-tier cap badge when the user is at the cap', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-settings-shareables-cap-badge')).toBeInTheDocument();
    });
  });

  it('hides the cap badge for pro users at the same shareable count', async () => {
    const api = createFakeSettingsApi({
      subscription: {
        userId: '11111111-2222-4222-8222-111111111111',
        tier: 'pro',
        source: 'revenuecat',
        externalCustomerId: 'rc_1',
    ...
    expect(screen.queryByTestId('m-settings-shareables-cap-badge')).toBeNull();
  });
  ...
  it('disables the create button while at the free-tier cap', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-settings-shareables-create-button')).toBeInTheDocument();
    });
    const btn = screen.getByTestId('m-settings-shareables-create-button');
  ...
  it('appends a newly-created shareable to the list', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-settings-shareables-create-slug')).toBeInTheDocument();
    });
    const input = screen.getByTestId('m-settings-shareables-create-slug') as HTMLInputElement;
    await act(async () => {
  ...
  it('shows the error state when the initial fetch fails', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-settings-shareables-error')).toBeInTheDocument();
    });
    expect(screen.getByTestId('m-settings-shareables-error').textContent).toContain('boom');
  });
  ...
  it('optimistically updates the profile then commits the server response', async () => {
    ...
    const handleInput = screen.getByTestId('m-profile-handle-input') as HTMLInputElement;
    await act(async () => {
      fireEvent.change(handleInput, { target: { value: 'pablo-2' } });
    });
    await waitFor(() => {
      expect(screen.getByTestId('m-profile-handle-availability').textContent).toContain(
        'available',
      );
    });
  ...
  it('rolls back the profile on a save error', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('m-profile-fields-error')).toBeInTheDocument();
    });
  ...
  it('renders the per-shareable row editor with the public URL preview', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId(`m-shareable-row-${FAKE_SHAREABLE.id}`)).toBeInTheDocument();
    });
    expect(screen.getByTestId('m-shareable-row-url').textContent).toBe(
      'binderly.app/c/pablo/my-binder',
    );
  });
  ...
  it('persists a toggle change to the server and updates state on success', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-shareable-row-show-missing')).toBeInTheDocument();
    });
    const checkbox = screen.getByTestId('m-shareable-row-show-missing') as HTMLInputElement;
    await act(async () => {
  ...
  it('shows the delete-confirm copy after the first delete click', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-shareable-row-delete')).toBeInTheDocument();
    });
    const del = screen.getByTestId('m-shareable-row-delete');
    await act(async () => {
      fireEvent.click(del);
    });
    expect(screen.getByTestId('m-shareable-row-delete-confirm-copy')).toBeInTheDocument();
  });
  ...
  it('rolls back the list on a delete error', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('m-shareable-row-error')).toBeInTheDocument();
    });
  ...
  it('rolls back a per-row save on update error', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-shareable-row-show-photos')).toBeInTheDocument();
    });
    const checkbox = screen.getByTestId('m-shareable-row-show-photos') as HTMLInputElement;
    await act(async () => {
      fireEvent.click(checkbox);
    });
    const save = screen.getByTestId('m-shareable-row-save');
  ...
  it('disables non-default theme options for free users', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-shareable-row-theme-select')).toBeInTheDocument();
    });
    const goldOpt = screen.getByTestId('m-shareable-row-theme-option-gold') as HTMLOptionElement;
    expect(goldOpt.disabled).toBe(true);
    expect(goldOpt.textContent).toContain('Pro');
    const defaultOpt = screen.getByTestId(
      'm-shareable-row-theme-option-default',
  ...
  it('disables the "show collection value" toggle for free users', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-shareable-row-show-values')).toBeInTheDocument();
    });
    const checkbox = screen.getByTestId('m-shareable-row-show-values') as HTMLInputElement;
  ...
  it('debounces the handle-availability check', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-profile-handle-input')).toBeInTheDocument();
    });
    const handleInput = screen.getByTestId('m-profile-handle-input') as HTMLInputElement;
    await act(async () => {
      fireEvent.change(handleInput, { target: { value: 'taken-handle' } });
    });
    expect(screen.getByTestId('m-profile-handle-availability').textContent).toContain('Checking');
    await waitFor(
      () => {
        expect(screen.getByTestId('m-profile-handle-availability').textContent).toContain(
          'already taken',
  ...
  it('blocks profile save when bio exceeds 280 chars', async () => {
    ...
    renderWithProvider(<ShareablesSettingsScreen api={api} />);
    await waitFor(() => {
      expect(screen.getByTestId('m-profile-bio-input')).toBeInTheDocument();
    });
    const bio = screen.getByTestId('m-profile-bio-input') as HTMLTextAreaElement;
    ...
    expect(screen.getByTestId('m-profile-bio-counter').textContent).toContain('limited to 280');
  });
