"""CornersModel — regression model for the corners sub-grade.
...
Production architecture (``CORNERS_USE_TORCH=1``)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Process 4 corner patches independently through the shared backbone, average
the 4 logit outputs, clamp to [1.0, 10.0].  Each patch is 64×64 RGB (224×224
for ImageNet-pretrained transfer).
...
class CornersModel:
    """Numpy linear regression placeholder for the corners sub-grade.

    ``input_dim`` should match the flattened feature vector produced by
    ``CornersDataset``.  In the CI smoke test this is
    ``NUM_CORNERS * patch_size * patch_size * 3``  (e.g. 4*8*8*3 = 768).
