"""Corners sub-grade model evaluation entry point.

Usage::

    python -m grading.corners.eval --model corners_v0.onnx
...
from grading.ml_common.eval_metrics import confusion_matrix_grades, summarise_metrics
from grading.ml_common.model_export import load_onnx_session, run_onnx_inference


def eval_corners_model(
    model_path: str | Path,
    ...
    """Evaluate a trained ONNX corners model on labelled data.
    ...
    if not labelled:
        raise ValueError("No labelled corners samples found.")
...
def _main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate corners sub-grade model")
    parser.add_argument("--model", type=str, required=True)
    parser.add_argument("--patch-size", type=int, default=64)
    args = parser.parse_args()

    print("corners/eval.py: Evaluating with synthetic data (no live DB in v0).")
    metrics = eval_corners_model(
