"""Corners sub-grade model ONNX export entry point.

Usage::

    python -m grading.corners.export --model-pkl corners_params.npz --output corners_v0.onnx

Or from train.py which calls ``export_corners_model`` directly.
"""
...
def export_corners_model(
    ...
    """Export a trained ``CornersModel`` to ONNX.

    Args:
        model: Trained ``CornersModel``.
...
def _main() -> None:
    parser = argparse.ArgumentParser(description="Export corners ONNX model")
    ...
    model = CornersModel(input_dim=args.input_dim)
    model.set_parameters({"W": params["W"], "b": params["b"]})
    out = export_corners_model(model, args.output, input_dim=args.input_dim)
