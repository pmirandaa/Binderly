"""Edges sub-grade model training entry point.

Usage::

    python -m grading.edges.train --epochs 10 --lr 0.001 --output edges_v0.onnx
...
6. Export to ONNX via ``export_edges_model``.
"""
...
def train_edges_model(
    ...
    """Train the edges model on labelled data from the three sources.
    ...
    Args:
        ...
        patch_size: Strip patch side length in pixels (8 for CI smoke test).
    ...
    if not labelled:
        raise ValueError("No labelled edges samples found in the provided data.")
    ...
    model = EdgesModel(input_dim=dataset.input_dim, random_seed=config.random_seed)
    loop = TrainingLoop(model=model, loss_fn=mse_loss, config=config)
    result = loop.run(X_train, y_train, X_val, y_val)

    if output_path is not None:
        export_edges_model(model, output_path, input_dim=dataset.input_dim)

    return result


def _main() -> None:
    parser = argparse.ArgumentParser(description="Train edges sub-grade model")
    ...
    print("edges/train.py: No live DB connection in this v0 — run with synthetic data.")
    print("Set EDGES_LIVE_IMAGES=1 and provide DB_URL for real training.")
    config = TrainingConfig(
        subgrade_column="edges",
