"""Shared fixtures for edges tests."""
...
def _make_psa_rows(n: int = 8) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        grade = float(5 + (i % 6))
        edges = round(grade - 0.5 + (i % 2) * 0.5, 1)
        rows.append({
            "source_id": f"PSA-{i:05d}",
            "grade_company": "PSA",
            "grade": str(grade),
            "subgrades": {"edges": edges, "centering": grade},
...
def _make_auction_rows(n: int = 10) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        grade = float(7 + (i % 4))
        edges = round(grade - 0.5 + (i % 3) * 0.25, 1)
        rows.append({
            "lot_id": f"PWCC-{i:04d}",
            "auction_house": "pwcc",
            "parsed_grading_company": "PSA",
...
def dataset(labelled_samples, mock_loader):
    return EdgesDataset(labelled_samples, image_loader=mock_loader, patch_size=PATCH_SIZE)


@pytest.fixture
def model():
    return EdgesModel(input_dim=INPUT_DIM, random_seed=42)
...
def trained_model(dataset):
    ...
    m = EdgesModel(input_dim=dataset.input_dim, random_seed=42)
...
def tmp_onnx_path(tmp_path):
    return tmp_path / "edges_test.onnx"
