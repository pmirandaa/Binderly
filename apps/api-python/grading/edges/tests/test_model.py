...
class TestEdgesModel:
    ...
    def test_forward_clamped_lower_bound(self, model):
        X = np.ones((3, INPUT_DIM), dtype=np.float32) * -1000.0
        out = model.forward(X)
        assert out.min() >= 1.0
    ...
    def test_backward_changes_bias(self, model):
        b_before = model.b.copy()
        X = np.random.default_rng(2).random((4, INPUT_DIM), dtype=np.float32)
        preds = model.forward(X)
        grad = np.ones_like(preds) * 0.1
        model._backward(X, grad, learning_rate=0.01)
        assert not np.array_equal(model.b, b_before)
    ...
    def test_deterministic_init(self):
        m1 = EdgesModel(input_dim=INPUT_DIM, random_seed=7)
        m2 = EdgesModel(input_dim=INPUT_DIM, random_seed=7)
        np.testing.assert_array_equal(m1.W, m2.W)

    def test_different_seeds_different_weights(self):
        m1 = EdgesModel(input_dim=INPUT_DIM, random_seed=1)
        m2 = EdgesModel(input_dim=INPUT_DIM, random_seed=2)
        assert not np.array_equal(m1.W, m2.W)

    def test_output_dim_attribute(self):
        m = EdgesModel(input_dim=INPUT_DIM, output_dim=1)
    ...
    def test_batch_forward_consistency(self, model):
        rng = np.random.default_rng(99)
        X = rng.random((10, INPUT_DIM), dtype=np.float32)
        batch_out = model.forward(X)
        single_outs = [model.forward(X[i : i + 1])[0] for i in range(10)]
        np.testing.assert_allclose(batch_out, single_outs, atol=1e-5)

    def test_input_dim_attribute(self):
        m = EdgesModel(input_dim=48)
        assert m.input_dim == 48

    def test_parameters_keys(self, model):
        params = model.get_parameters()
        assert "W" in params
        assert "b" in params
