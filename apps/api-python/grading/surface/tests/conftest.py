...
PATCH_SIZE = 8
INPUT_DIM = NUM_SURFACE_SHOTS_V1 * PATCH_SIZE * PATCH_SIZE * 3  # 384
...
def _make_psa_rows(n: int = 8) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        grade = float(5 + (i % 6))
        surface = round(grade - 0.5 + (i % 2) * 0.5, 1)
        rows.append({
            "source_id": f"PSA-{i:05d}",
            "grade_company": "PSA",
            "grade": str(grade),
            "subgrades": {"surface": surface, "centering": grade, "corners": grade - 0.5},
            "images": {
                "front": f"mock://psa/{i}_front.jpg",
                "back": f"mock://psa/{i}_back.jpg",
            },
            "printing_id": None,
            "raw_metadata": {},
        })
...
def _make_auction_rows(n: int = 10) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        grade = float(7 + (i % 4))
        surface = round(grade - 0.5 + (i % 3) * 0.25, 1)
        rows.append({
            "lot_id": f"PWCC-{i:04d}",
            "auction_house": "pwcc",
            "parsed_grading_company": "PSA",
            "parsed_overall_grade": str(grade),
            "parsed_sub_grades": {"surface": surface},
            "lot_image_urls": [f"mock://pwcc/{i}_front.jpg", f"mock://pwcc/{i}_back.jpg"],
            "printing_id": None,
...
def _make_rows_without_surface(n: int = 5) -> list[dict[str, Any]]:
    """Rows that have no surface subgrade — should be filtered out."""
    rows = []
    for i in range(n):
        rows.append({
            "source_id": f"PSA-NS-{i:05d}",
            "grade_company": "PSA",
            "grade": "9",
            "subgrades": {"corners": 9.0},  # no surface key
            "images": {"front": f"mock://psa/ns_{i}.jpg"},
            "printing_id": None,
            "raw_metadata": {},
        })
    return rows
...
def dataset(labelled_samples, mock_loader):
    return SurfaceDataset(labelled_samples, image_loader=mock_loader, patch_size=PATCH_SIZE)


@pytest.fixture
def model():
    return SurfaceModel(input_dim=INPUT_DIM, random_seed=42)
...
def trained_model(dataset):
    ...
    m = SurfaceModel(input_dim=dataset.input_dim, random_seed=42)
...
def tmp_onnx_path(tmp_path):
    return tmp_path / "surface_test.onnx"
