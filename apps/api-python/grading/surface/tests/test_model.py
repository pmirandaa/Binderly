"""Tests for surface/model.py — SurfaceModel."""
...
class TestSurfaceModel:
    def test_forward_output_shape(self, model):
        X = np.random.default_rng(0).random((5, INPUT_DIM)).astype(np.float32)
        out = model.forward(X)
        assert out.shape == (5,)
    ...
    def test_forward_negative_large_clamped(self, model):
        X = np.ones((2, INPUT_DIM), dtype=np.float32) * -1000.0
        out = model.forward(X)
        assert out.min() >= 1.0

    def test_backward_changes_weights(self, model):
        W_before = model.W.copy()
        X = np.random.default_rng(1).random((4, INPUT_DIM)).astype(np.float32)
    ...
    def test_backward_changes_bias(self, model):
        ...
        X = np.random.default_rng(2).random((4, INPUT_DIM)).astype(np.float32)
        preds = model.forward(X)
        grad = np.ones_like(preds) * 0.1
        model._backward(X, grad, learning_rate=0.01)
    ...
    def test_deterministic_init(self):
        m1 = SurfaceModel(input_dim=INPUT_DIM, random_seed=7)
        m2 = SurfaceModel(input_dim=INPUT_DIM, random_seed=7)
        np.testing.assert_array_equal(m1.W, m2.W)

    def test_different_seeds_different_weights(self):
        m1 = SurfaceModel(input_dim=INPUT_DIM, random_seed=1)
        m2 = SurfaceModel(input_dim=INPUT_DIM, random_seed=2)
        assert not np.array_equal(m1.W, m2.W)

    def test_output_dim_attribute(self):
        m = SurfaceModel(input_dim=INPUT_DIM, output_dim=1)
    ...
    def test_input_dim_attribute(self):
        m = SurfaceModel(input_dim=INPUT_DIM)
        assert m.input_dim == INPUT_DIM
